...
- Publish the Codex group prices for `gpt-5.5` and `gpt-5.4`.
- `group_name` is the stable public group identifier `codex`.
...
That preserves the existing service fallback behavior where `gpt-5.5` and `gpt-5.4` resolve to the built-in GPT-5.4 static pricing when the remote LiteLLM catalog is missing or stale.
