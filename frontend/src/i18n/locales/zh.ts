  ...
  points: {
    ...
    admin: {
      config: {
        title: '积分配置',
        enabled: '启用积分功能',
        enabledHint: '与旧版「邀请返利（$ 返利）」互斥：启用积分前须先在系统设置关闭 affiliate_enabled，否则保存会被拒绝。',
        peg: '面值（每积分折合余额单位）',
        pegHint: '例如 0.01 表示 100 积分 = 1 余额单位。上线后视为不可变。',
        pegChangeConfirm: '确定把面值从 {from} 改为 {to}？这会重估全部存量积分的价值，且此变更会被审计留痕。',
        cashbackRate: '返积分比例（%）',
        freezeHours: '冻结期（小时）',
        withdrawEnabled: '启用提现',
        withdrawMin: '最低提现积分',
        withdrawFee: '提现手续费（%）',
        redeemBalanceOn: '允许兑换余额',
        redeemPlanOn: '允许兑换套餐',
        save: '保存',
        saved: '配置已保存。',
      },
