...
describe('useAppStore', () => {
  ...
  describe('公开设置加载', () => {
    it('并发普通与 force 调用复用同一个未决请求', async () => {
    ...
    it('force 绕过已结算缓存，刷新期间的普通调用复用刷新请求', async () => {
    ...
    it('并发请求失败时所有调用返回 null 且不标记已加载', async () => {
      const deferred = createDeferred<PublicSettings>()
      vi.mocked(getPublicSettings).mockReturnValue(deferred.promise)
      ...
      await expect(Promise.all([first, forced])).resolves.toEqual([null, null])
      expect(getPublicSettings).toHaveBeenCalledTimes(1)
      expect(store.publicSettingsLoaded).toBe(false)
      expect(store.cachedPublicSettings).toBeNull()
    })

    it('API 同步抛错时返回 null 且不标记已加载', async () => {
      ...
      await expect(store.fetchPublicSettings()).resolves.toBeNull()
