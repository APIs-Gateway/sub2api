...
describe("admin SettingsView payment visible method controls", () => {
  ...
  it("disables panel rate limit threshold inputs when the master switch is off", async () => {
    getPanelRateLimitSettings.mockResolvedValue({
      enabled: false,
      user_rpm: 240,
      heavy_rpm: 60,
  ...
  it("exposes the force priority OpenAI fast policy action", async () => {
    ...
    const actionOptions = wrapper
      .findAll("option")
      .filter((node) => node.attributes("value") === "force_priority");
