...
// fork 版订阅页（按日额度分配，无分组选择）：上游 c76db386c / 51c5d0388 的用户搜索用例按 fork 表单改写。
describe('admin subscription user search', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    localStorage.clear()
  ...
  it.each(['another', ''])('clears the assignment user immediately when input changes to %j', async (keyword) => {
    ...
    try {
      ...
      expect(assignSubscription).toHaveBeenCalledWith({
        user_id: 84, daily_amount_usd: 30, validity_days: 30
      })
    } finally {
      wrapper.unmount()
      vi.useRealTimers()
