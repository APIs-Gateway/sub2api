    ...
    <div class="mx-auto max-w-6xl space-y-6">
      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
          ...
          <div class="card">
            ...
            <div class="space-y-5 p-6">
              ...
              >
                <div>
                  <label class="font-medium text-gray-900 dark:text-white">{{
                    t("admin.settings.registration.sessionBinding")
                  }}</label>
                  <p class="text-sm text-gray-500 dark:text-gray-400">
                    {{ t("admin.settings.registration.sessionBindingHint") }}
                  </p>
                </div>
            ...
            <div class="space-y-5 p-6">
              ...
              >
                <div class="min-w-0">
                  ...
                  <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                    {{ t("admin.settings.openaiExperimentalScheduler.oauthRateDescription") }}
                  </p>
                </div>
                <input
              ...
              >
                <div class="min-w-0">
                  <label
                    class="text-sm font-medium text-gray-700 dark:text-gray-300"
                    for="openai-advanced-scheduler-weight-upstream-cost"
                  >
                    {{ t("admin.settings.openaiExperimentalScheduler.upstreamCostWeightTitle") }}
                  </label>
                  <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                    {{ t("admin.settings.openaiExperimentalScheduler.upstreamCostWeightDescription") }}
                  </p>
                </div>
                <input
                  id="openai-advanced-scheduler-weight-upstream-cost"
                  v-model="form.openai_advanced_scheduler_weight_upstream_cost"
                  class="input w-full shrink-0 sm:w-32"
                  data-testid="openai-advanced-scheduler-weight-upstream-cost"
                  min="0"
                  placeholder="0"
                  step="0.1"
                  type="number"
        ...
        <div class="card">
          <div class="border-b border-gray-100 px-6 py-4 dark:border-dark-700">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
              {{ t('admin.settings.features.regionNotice.title') }}
            </h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
              {{ t('admin.settings.features.regionNotice.description') }}
            </p>
          </div>
          <div class="space-y-5 p-6">
            <div class="flex items-center justify-between gap-4">
              <div>
                <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                  {{ t('admin.settings.features.regionNotice.enabled') }}
                </label>
                <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                  {{ t('admin.settings.features.regionNotice.enabledHint') }}
                </p>
              </div>
              <Toggle
                v-model="form.user_region_notice_enabled"
                data-testid="user-region-notice-enabled"
              />
            </div>
          </div>
        </div>
...
> & {
  ...
  /** "" (cleared input) or null means OAuth accounts use their own account rates. */
...
const form = reactive<SettingsForm>({
  ...
  // User dashboard region notice feature switch
...
async function saveSettings() {
  ...
  try {
    ...
    const payload: UpdateSettingsRequest = {
      ...
      enable_client_dateline_normalization: form.enable_client_dateline_normalization,
      antigravity_user_agent_version:
      ...
      openai_advanced_scheduler_weight_upstream_cost:
        String(form.openai_advanced_scheduler_weight_upstream_cost ?? ""),
      // 余额、订阅到期与账号限额通知
      ...
      // User dashboard region notice feature switch
      user_region_notice_enabled: form.user_region_notice_enabled,
      // Affiliate (邀请返利) feature switch
    ...
    if ((error as { reason?: string })?.reason === "STEP_UP_ENABLE_REQUIRES_TOTP") {
      appStore.showError(t("admin.settings.registration.stepUpEnableRequiresTotp"));
      return;
    }
    appStore.showError(
...
const providerKeyOptions = computed(() => [
  ...
  { value: "crypto", label: t("admin.settings.payment.providerCrypto") },
]);
...
.settings-tab-active::after {
  ...
  content: "";
  background: linear-gradient(90deg, #cc785c, #0ea5e9);
}
