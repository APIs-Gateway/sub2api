...
// Use the same breakpoint as the other ops cards so wide log tables do not overflow on phones.
// c8 ignore next: the browser media-query binding is covered by the responsive template path.
...
const cleanupCurrentFilter = async () => {
  ...
  try {
    ...
    appStore.showError(
      extractApiErrorMessage(err, err?.response?.data?.detail || '清理系统日志失败', {
    ...
    <div class="mb-4 rounded-xl border border-gray-200 bg-gray-50 p-3 dark:border-dark-700 dark:bg-dark-800/70">
      ...
      <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-6">
        ...
        <label class="text-xs text-gray-600 dark:text-gray-300">
          ...
          <span class="mt-1 block text-[11px] text-gray-500 dark:text-gray-400">由定时数据清理任务执行。</span>
        </label>
      ...
      <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">默认关闭，因为访问日志会为每个请求新增一条带索引的数据库记录。警告、错误和审计日志始终会保留。</p>
    ...
    <div class="overflow-hidden rounded-xl border border-gray-200 dark:border-dark-700">
      ...
      <div v-else-if="!isDesktopViewport" class="divide-y divide-gray-100 dark:divide-dark-800">
        <div v-for="row in logs" :key="row.id" class="space-y-1.5 p-3">
          ...
          <div v-if="row.component" class="truncate text-xs text-gray-500 dark:text-gray-400" :title="row.component">
            {{ row.component }}
          </div>
