...
const loadStats = async () => { loading.value = true; loadError.value = false; try { const [dashboardStats] = await Promise.all([usageAPI.getDashboardStats(), authStore.refreshUser().catch((error) => { console.warn('Failed to refresh user on dashboard load:', error) })]); stats.value = dashboardStats } catch (error) { loadError.value = true; console.error('Failed to load dashboard stats:', error) } finally { loading.value = false } }
