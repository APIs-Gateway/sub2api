...
const loadStats = async () => { loading.value = true; loadError.value = false; try { stats.value = await usageAPI.getDashboardStats() } catch (error) { loadError.value = true; console.error('Failed to load dashboard stats:', error) } finally { loading.value = false } }
