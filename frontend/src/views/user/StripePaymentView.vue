  ...
  <component :is="isPopup ? 'div' : AppLayout" :class="isPopup ? 'min-h-screen bg-gray-50 dark:bg-dark-900' : ''">
    <div class="mx-auto max-w-lg space-y-6 py-8" :class="isPopup ? 'px-4' : ''">
      ...
      <div v-else-if="initError" class="card p-8 text-center">
        <div class="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full border border-primary-200 bg-primary-50 dark:border-primary-700/50 dark:bg-primary-900/20">
          <Icon name="exclamationCircle" size="xl" class="text-primary-700 dark:text-primary-400" />
        </div>
      ...
      <template v-else>
        ...
        <template v-if="wechatQrUrl">
          <div class="card p-6">
            <div class="flex flex-col items-center space-y-4">
              ...
              <div class="relative rounded-md border border-gray-200 bg-white p-4 dark:border-dark-700 dark:bg-dark-800">
                <img :src="wechatQrUrl" alt="WeChat Pay QR" class="h-56 w-56 rounded" />
                <div class="pointer-events-none absolute inset-0 flex items-center justify-center">
                  <span class="rounded-full bg-[#2BB741] p-2 ring-2 ring-white">
              ...
              <p class="text-center text-sm text-gray-600 dark:text-gray-400">{{ t('payment.qr.scanWxpayHint') }}</p>
            </div>
          </div>
          <div class="card p-4 text-center">
            <p class="text-sm text-gray-600 dark:text-gray-400">{{ t('payment.qr.waitingPayment') }}</p>
          </div>
        </template>
        ...
        <template v-else-if="redirecting">
          <div class="card p-6">
            <div class="flex flex-col items-center space-y-4 py-4">
              <div class="h-10 w-10 animate-spin rounded-full border-4 border-gray-400 border-t-transparent dark:border-gray-500"></div>
              <p class="text-sm text-gray-600 dark:text-gray-400">{{ t('payment.qr.payInNewWindowHint') }}</p>
            </div>
          </div>
        </template>
        ...
        <template v-else-if="stripeSuccess">
          <div class="card p-6 text-center">
            <div class="flex flex-col items-center gap-3 py-4">
              <div class="flex h-16 w-16 items-center justify-center rounded-full border border-gray-200 bg-gray-50 dark:border-dark-700 dark:bg-dark-800">
                <Icon name="check" size="lg" class="text-gray-900 dark:text-white" />
              </div>
              <p class="text-lg font-bold text-gray-900 dark:text-white">{{ t('payment.result.success') }}</p>
              <p class="text-sm text-gray-600 dark:text-gray-400">{{ t('payment.stripeSuccessProcessing') }}</p>
            </div>
          </div>
        </template>
        ...
        <template v-else-if="showPaymentElement">
          <div class="card p-6">
            ...
            <p v-if="stripeError" class="mt-4 text-sm text-primary-700 dark:text-primary-400">{{ stripeError }}</p>
        ...
        <div v-if="stripeError && !showPaymentElement" class="card p-4">
          <p class="text-sm text-primary-700 dark:text-primary-400">{{ stripeError }}</p>
