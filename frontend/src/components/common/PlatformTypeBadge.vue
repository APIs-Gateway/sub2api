...
// Neutral hairline chip — platforms distinguished by label/icon text only.
// Rendered as a single segmented pill: outer border on the leading segment,
// a hairline divider (left border) on the trailing segment to avoid a double seam.
const neutralChip = 'border border-gray-200 text-gray-700 dark:border-dark-700 dark:text-gray-300'
const neutralSegment = 'border-l border-gray-200 text-gray-700 dark:border-dark-700 dark:text-gray-300'

const platformClass = computed(() => neutralChip)

const typeClass = computed(() => neutralSegment)

const planBadgeClass = computed(() => {
  // Signal (clay) only for genuine abnormal/over-limit state
  if (props.planType && props.planType.toLowerCase() === 'abnormal') {
    return 'border border-primary-200 text-primary-700 dark:border-primary-800 dark:text-primary-400'
  }
  return neutralChip
})
...
const privacyBadge = computed(() => {
  ...
  switch (props.privacyMode) {
    // OpenAI states
    case 'training_off':
      return { label: 'Private', icon: shieldCheck, title: t('admin.accounts.privacyTrainingOff'), class: 'border border-gray-200 text-gray-700 dark:border-dark-700 dark:text-gray-300' }
    case 'training_set_cf_blocked':
      return { label: 'CF', icon: shieldX, title: t('admin.accounts.privacyCfBlocked'), class: 'border border-gray-200 text-gray-700 dark:border-dark-700 dark:text-gray-300' }
    case 'training_set_failed':
      return { label: 'Fail', icon: shieldX, title: t('admin.accounts.privacyFailed'), class: 'border border-primary-200 text-primary-700 dark:border-primary-800 dark:text-primary-400' }
    // Antigravity states
    case 'privacy_set':
      return { label: 'Private', icon: shieldCheck, title: t('admin.accounts.privacyAntigravitySet'), class: 'border border-gray-200 text-gray-700 dark:border-dark-700 dark:text-gray-300' }
    case 'privacy_set_failed':
      return { label: 'Fail', icon: shieldX, title: t('admin.accounts.privacyAntigravityFailed'), class: 'border border-primary-200 text-primary-700 dark:border-primary-800 dark:text-primary-400' }
    default:
      return null
  }
})
