...
describe('PaymentProviderDialog payment guide', () => {
  ...
  // Fork-specific: this fork has a built-in "crypto" (BEpusdt) payment type that
  // upstream does not have. GetBasePaymentType prefix-collapses any "crypto*" type
  // into "crypto", so a custom EasyPay method starting with "crypto" would silently
  // merge into the unrelated crypto provider's limits/visibility if allowed.
  it('rejects custom EasyPay method types with the built-in crypto prefix', async () => {
    const provider = providerFactory({
      provider_key: 'easypay',
      name: 'EasyPay',
      config: {
    ...
    await typeInput.setValue('crypto_usdt')
    await upstreamTypeInput.setValue('usdt')
    await displayNameInput.setValue('Crypto USDT')
    await wrapper.find('form').trigger('submit.prevent')
