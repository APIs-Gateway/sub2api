  ...
  <div class="card p-4">
    <div class="mb-3 flex flex-wrap items-center gap-3">
      ...
      <div class="ml-auto flex flex-wrap items-center gap-2">
        <input v-model="search" type="text" class="input h-8 w-44 text-sm" :placeholder="t('admin.usage.tokenRanking.searchPlaceholder')" />
        <div class="w-28">
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-gray-100 text-xs text-gray-500 dark:border-gray-700 dark:text-gray-400">
            ...
            <th v-for="col in sortableColumns" :key="col.key" class="cursor-pointer select-none py-2 pl-2 text-right font-medium hover:text-primary-500" :class="{ 'text-primary-600 dark:text-primary-400': sortBy === col.key }" @click="setSort(col.key)">
              {{ t(col.label) }}
        ...
        <tbody>
          <tr v-if="loading">
            <td :colspan="sortableColumns.length + 2" class="py-8 text-center"><LoadingSpinner /></td>
          </tr>
          <tr v-else-if="displayItems.length === 0">
            <td :colspan="sortableColumns.length + 2" class="py-8 text-center text-sm text-gray-400">{{ t('admin.dashboard.noDataAvailable') }}</td>
          </tr>
          <tr v-for="(item, index) in displayItems" v-else :key="item.user_id" class="cursor-pointer border-b border-gray-50 transition-colors hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-dark-700/40" @click="$emit('select-user', item.user_id, item.email)">
            <td class="py-2 pr-2 text-gray-400">{{ index + 1 }}</td>
            <td class="max-w-[220px] truncate py-2 pr-2 text-gray-700 dark:text-gray-200" :title="item.email">{{ item.email || `User #${item.user_id}` }}</td>
...
const displayItems = computed(() => {
  ...
  return items.value.filter((user) => (user.email || `user #${user.user_id}`).toLowerCase().includes(keyword))
})
