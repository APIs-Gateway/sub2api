...
import { canRefund, statusBadgeClass } from '@/components/payment/orderUtils'
...
describe('admin order currency display', () => {
  ...
  it('shows provider-pending refunds as warning and prevents duplicate refund actions', () => {
    expect(statusBadgeClass('REFUND_PENDING')).toBe('badge-warning')
    expect(canRefund('REFUND_PENDING')).toBe(false)
  })
})
