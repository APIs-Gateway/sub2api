...
/**
 * API base URL shared with `client.ts`. It lives here (instead of client.ts) so this module can
 * call the refresh endpoint with bare axios without importing the intercepted client.
 */
...
async function requestTokenPair(
  ...
  try {
    ...
    // 429 is rate limiting, not a token rejection: the fork surfaces it as TOKEN_REFRESH_UNAVAILABLE
    // (session kept), so it only gets the short reconciliation window.
