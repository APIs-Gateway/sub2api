...
describe('refreshAuthTokens', () => {
  ...
  it('treats a rate-limited refresh as transient instead of waiting for an uncoordinated peer', async () => {
    vi.useFakeTimers()
    seedSession()
    const rateLimited = { response: { status: 429 } }
    mockedPost.mockRejectedValueOnce(rateLimited)
    const { refreshAuthTokens } = await import('@/api/tokenRefresh')

    let settled = false
    const pending = refreshAuthTokens({ failedAccessToken: 'old-access' })
    const rejection = expect(pending).rejects.toBe(rateLimited)
    void pending.catch(() => undefined).finally(() => {
      settled = true
    })

    // Without Web Locks a 401/403 waits through the full peer deadline (~31s); 429 must not.
  ...
  it('posts the current refresh token to the configured refresh endpoint with a 30s timeout', async () => {
    seedSession()
    mockedPost.mockResolvedValueOnce(refreshedResponse())
    const { refreshAuthTokens, API_BASE_URL } = await import('@/api/tokenRefresh')
