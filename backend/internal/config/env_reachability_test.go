...
// TestConfigKeysAreEnvReachable is the systemic guard behind the
// image-storage-env-unreachable class of bug (upstream #4593): viper.Unmarshal
// only decodes keys returned by AllKeys(), which unions SetDefault keys,
// config-file keys and explicit BindEnv keys. AutomaticEnv can override a key
// already in that union but never introduces one, and the viper_bind_struct
// escape hatch is compiled out (we build with -tags embed).
//
...
// as if it were never configured. deploy/docker-compose.yml is exactly this
// kind of env-driven deployment.
//
