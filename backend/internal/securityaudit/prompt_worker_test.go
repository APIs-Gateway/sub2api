...
func workerTestResult(decision EventDecision) *NormalizedResult {
	return &NormalizedResult{
		Decision: decision, RiskLevel: RiskLow, Action: ActionAllow,
...
func TestRunnerWorkerTicksAndPublishesHeartbeat(t *testing.T) {
	cfg := asyncConfig()
	cfg.WorkerCount = 1
	repo := &fakeJobRepository{}
	...
	active, _, _, heartbeat, _, _, _ := runner.Snapshot()
