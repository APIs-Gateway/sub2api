...
func TestCoordinatorAsyncEnqueueFailuresNeverChangeResponseOrDownstreamDispatch(t *testing.T) {
	for _, enqueueErr := range []error{errors.New("queue full"), errors.New("queue admission busy"), errors.New("redis unavailable"), errors.New("publish failed")} {
...
func TestCoordinatorNilAndLegacyFallbackBranches(t *testing.T) {
	var coordinator *Coordinator
	decision := coordinator.Check(context.Background(), Request{})
	require.Equal(t, DecisionAllow, decision.Kind)

	decision = NewCoordinator(nil, nil).Check(context.Background(), Request{})
	require.Equal(t, DecisionAllow, decision.Kind)

	legacy := &fakeLegacyEngine{decision: &LegacyDecision{Blocked: true, Message: "blocked"}}
	decision = NewCoordinator(legacy, nil).Check(context.Background(), Request{})
	require.Equal(t, DecisionBlock, decision.Kind)
	require.Equal(t, http.StatusForbidden, decision.HTTPStatus)
	require.Equal(t, "content_policy_violation", decision.ErrorCode)
}

func TestCoordinatorMapsInvalidPromptAndNilPromptResults(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "invalid response", err: invalid, want: DecisionInvalid},
		{name: "nil result", err: nil, want: DecisionUnavailable},
	} {
		t.Run(test.name, func(t *testing.T) {
			prompt := &fakePromptEngine{mode: ModeBlocking, err: test.err}
			if test.name == "nil result" {
				prompt.err = nil
			}
			decision := NewCoordinator(nil, prompt).Check(context.Background(), Request{})
			require.Equal(t, test.want, decision.Kind)
		})
	}
}
