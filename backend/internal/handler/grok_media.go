...
func (h *OpenAIGatewayHandler) handleGrokMedia(c *gin.Context, endpoint service.GrokMediaEndpoint, requestID string) {
	...
	if endpoint.IsGenerationRequest() {
		if !service.GroupAllowsImageGeneration(apiKey.Group) {
			...
			if endpoint == service.GrokMediaEndpointVideosGenerations {
				message = service.VideoGenerationPermissionMessage()
			}
			h.errorResponse(c, http.StatusForbidden, "permission_error", message)
			return
		}
		if moderationBody := grokMediaModerationBody(contentType, body); len(moderationBody) > 0 {
	...
	if endpoint == service.GrokMediaEndpointVideoStatus {
		// GenerateExplicitSessionHash intentionally ignores arbitrary body values;
		// use the xAI request ID directly as the stable account-affinity key.
		sessionHash = service.DeriveSessionHashFromSeed(requestID)
	} else {
...
func grokMediaModerationBody(contentType string, body []byte) []byte {
	if gjson.ValidBytes(body) {
		return body
	}
	return service.BuildOpenAIImagesModerationBody(contentType, body)
}
