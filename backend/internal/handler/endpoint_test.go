...
func TestDeriveUpstreamEndpoint(t *testing.T) {
	tests := []struct {
		...
		{"openai responses compact inbound only", EndpointResponsesCompact, "/v1/messages", service.PlatformOpenAI, EndpointResponsesCompact},
...
func TestResolveOpenAIUpstreamEndpoint_RawChatAPIKey(t *testing.T) {
...
func TestResolveOpenAIUpstreamEndpoint_ResponsesCapableOpenAI(t *testing.T) {
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/messages", nil)
