...
// TryAcquireUserSlotForAPIKey acquires the existing user admission slot and,
// once admitted, tracks the API key for best-effort real-time statistics.
...
func (h *ConcurrencyHelper) withAPIKeySlot(ctx context.Context, apiKeyID int64, releaseFunc func()) func() {
	...
	apiKeyRelease := h.concurrencyService.TrackAPIKeySlot(ctx, apiKeyID)
	return func() {
		if releaseFunc != nil {
			releaseFunc()
		}
