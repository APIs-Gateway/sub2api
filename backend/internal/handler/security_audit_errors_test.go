...
func TestPromptGuardAlternateProtocolErrorAdapters(t *testing.T) {
	gin.SetMode(gin.TestMode)
	for _, kind := range []securityaudit.DecisionKind{securityaudit.DecisionBlock, securityaudit.DecisionUnavailable, securityaudit.DecisionInvalid} {
		decision := promptGuardDecision(kind)
		t.Run("gateway_openai_"+string(kind), func(t *testing.T) {
			c, recorder := securityAuditErrorTestContext(t)
			(&GatewayHandler{}).openAISecurityAuditError(c, decision)
			require.Equal(t, decision.HTTPStatus, recorder.Code)
			require.Contains(t, recorder.Body.String(), decision.ErrorCode)
		})
		t.Run("openai_anthropic_"+string(kind), func(t *testing.T) {
			c, recorder := securityAuditErrorTestContext(t)
			(&OpenAIGatewayHandler{}).anthropicSecurityAuditError(c, decision)
			require.Equal(t, decision.HTTPStatus, recorder.Code)
			require.Contains(t, recorder.Body.String(), decision.ErrorCode)
		})
	}
...
func TestPromptGuardErrorHelpersCoverNilAndLegacyShapes(t *testing.T) {
	require.Equal(t, http.StatusForbidden, securityAuditStatus(nil))
	require.Equal(t, "content_policy_violation", securityAuditErrorCode(nil))
	require.Equal(t, "Request blocked by content policy", securityAuditMessage(nil))
	require.Nil(t, (legacyContentModerationDecision{}).toService())
	converted := (legacyContentModerationDecision{value: &securityaudit.LegacyDecision{
		Allowed: true, Blocked: false, Flagged: true, Message: "legacy", StatusCode: http.StatusOK, Action: "Warn",
	}}).toService()
	require.Equal(t, "legacy", converted.Message)
	require.True(t, converted.Flagged)
}
