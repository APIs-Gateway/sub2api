...
// TestOpenAIResponses_ForwardGenericErrorWithNilResultSkipsUsageRecord covers
// the defensive "if res == nil { return }" guard inside submitResponsesUsage:
// forwardOpenAIResponsesViaRawChatCompletions returns a plain (non-failover)
// error with a nil *OpenAIForwardResult when the account is missing its API
// key (see openai_gateway_responses_chat_fallback.go's "missing api_key"
// check). That generic-error branch still calls submitResponsesUsage(result)
// with result == nil, and the guard must no-op instead of dereferencing it.
func TestOpenAIResponses_ForwardGenericErrorWithNilResultSkipsUsageRecord(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	account := service.Account{
		ID:          9917,
		Name:        "openai-raw-responses-missing-api-key",
		Platform:    service.PlatformOpenAI,
		Type:        service.AccountTypeAPIKey,
		Status:      service.StatusActive,
		Schedulable: true,
		...
		GroupIDs:    []int64{groupID},
		Credentials: map[string]any{
			// 故意不设 api_key —— forwardOpenAIResponsesViaRawChatCompletions 会
			// 在联系上游之前就以 nil result + 普通 error 返回。
			"base_url": "https://api.openai.example",
		},
		Extra: map[string]any{
			openai_compat.ExtraKeyResponsesMode: string(openai_compat.ResponsesSupportModeForceChatCompletions),
