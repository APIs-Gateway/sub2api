...
// systemUpdateTimeout bounds the full in-place update while staying above the
// GitHub release download client's own timeout.
const systemUpdateTimeout = 15 * time.Minute

// systemUpdateContext keeps a slow release download alive after a browser or
// reverse proxy cancels the HTTP request, while retaining a hard upper bound.
