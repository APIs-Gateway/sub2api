...
var ingressRejectReasons = map[string]struct{}{
	...
	"invalid_auth_rate_limited": {}, "api_key_auth_overloaded": {},
...
// (see service.OpsIngressRejectAggregate). The underlying table never stores raw
// request bodies, headers, or credentials, and client_ip is already a masked network
// prefix (IPv4 /24, IPv6 /64; see service.maskClientIP), never a raw per-device
// address, so this endpoint cannot leak either.
...
func (h *OpsHandler) ListIngressRejects(c *gin.Context) {
	...
	// client_ip filters against the already-masked network prefix stored on each
	// row (e.g. "203.0.113.0/24"), not a raw address; pass through as-is.
...
// GetIngressRejectHealth exposes the in-process aggregator's buffering/flush health
// (cardinality budget usage, pending batches, drop/overflow counters).
