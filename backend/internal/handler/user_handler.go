...
// GetInviteCashback returns the current user's invite cashback details.
// GET /api/v1/user/invite-cashback
func (h *UserHandler) GetInviteCashback(c *gin.Context) {
	subject, ok := middleware2.GetAuthSubjectFromContext(c)
	if !ok {
		response.Unauthorized(c, "User not authenticated")
		return
	}
	detail, err := h.affiliateService.GetInviteCashbackDetail(c.Request.Context(), subject.UserID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
