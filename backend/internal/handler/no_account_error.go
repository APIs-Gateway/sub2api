...
type noAccountCapacityMarkMode uint8

const (
	noAccountCapacityMarkNone noAccountCapacityMarkMode = iota
...
// resolveNoAccountError keeps every protocol's selection-failure branch on
// the same 404-versus-503 decision. The caller supplies its legacy 503
// message and capacity-marking mode so a model configuration error is not
// recorded as a transient routing-capacity failure.
func resolveNoAccountError(
	c *gin.Context,
	diag service.ModelAvailabilityDiagnoser,
	apiKey *service.APIKey,
	routingModel string,
	...
	// Only ErrNoAvailableAccounts (or a nil selection) has the ambiguous
	// "model mapping versus temporary capacity" meaning that warrants a second
	// lookup. Repository/snapshot failures must retain their 503 semantics.
	if selectionErr == nil || errors.Is(selectionErr, service.ErrNoAvailableAccounts) {
		effectivePlatform := platform
		if c != nil && c.Request != nil {
			if forced, ok := c.Request.Context().Value(ctxkey.ForcePlatform).(string); ok && strings.TrimSpace(forced) != "" {
				effectivePlatform = forced
			} else if groupPlatform := strings.TrimSpace(service.PlatformFromAPIKey(apiKey)); groupPlatform != "" {
				// Invalid-request fallback can replace the API key/group after the
				// handler captured its initial platform. Follow the current key.
				effectivePlatform = groupPlatform
			}
		}
		cls = classifyNoAccountErrorFromGin(c, diag, apiKey, routingModel, displayModel, effectivePlatform)
	}
	...
	if c != nil {
		switch markMode {
		case noAccountCapacityMarkIfNoAvailable:
			markOpsRoutingCapacityLimitedIfNoAvailable(c, selectionErr)
		case noAccountCapacityMarkAlways:
			markOpsRoutingCapacityLimited(c)
		}
	}
	if fallbackMessage != "" {
		cls.Message = fallbackMessage
	}
	return cls
}
