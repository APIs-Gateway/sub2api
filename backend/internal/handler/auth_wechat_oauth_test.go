...
func TestWeChatPaymentOAuthCallbackRedirectsWithOpaqueResumeToken(t *testing.T) {
	...
	req.AddCookie(encodedCookie(wechatPaymentOAuthContextName, `{"payment_type":"wxpay","amount":"12.5","order_type":"subscription","plan_id":7,"daily_amount_usd":8.5,"validity_days":60}`))
	...
	require.InDelta(t, 8.5, claims.DailyAmountUSD, 1e-9)
	require.Equal(t, 60, claims.ValidityDays)
