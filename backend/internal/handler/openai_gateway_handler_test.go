...
// TestOpenAIResponsesWebSocket_BeforeRequestHookRecordsWSTurnInContext 覆盖
// ResponsesWebSocket 中 hooks.BeforeRequest 闭包真正被调用的路径：首个
// response.create 帧由握手阶段直接消费，不经过 proxyResponsesWebSocketV2Passthrough
// 的 filter；只有同一连接上的第二个 response.create 帧才会走 filter 并触发
// hooks.BeforeRequest，从而执行 c.Set(securityAuditWSTurnContextKey, turn)。
func TestOpenAIResponsesWebSocket_BeforeRequestHookRecordsWSTurnInContext(t *testing.T) {
	gin.SetMode(gin.TestMode)

	turnHitCh := make(chan []byte, 2)
	...
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		for i := 0; i < 2; i++ {
			...
			writeCtx, cancelWrite := context.WithTimeout(r.Context(), 3*time.Second)
			_ = conn.Write(writeCtx, coderws.MessageText, []byte(`{"type":"response.completed","response":{"id":"resp_ws_turn_ok","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`))
			cancelWrite()
		}
		_ = conn.Close(coderws.StatusNormalClosure, "done")
	}))
	...
	apiKey := &service.APIKey{
		ID:      1902,
		GroupID: &groupID,
		User:    &service.User{ID: 1802, Status: service.StatusActive},
		Group:   &service.Group{ID: groupID, Platform: service.PlatformOpenAI, Status: service.StatusActive},
	}
	...
	router.Use(func(c *gin.Context) {
		...
		c.Next()
		mu.Lock()
		capturedTurn, capturedExists = c.Get(securityAuditWSTurnContextKey)
		mu.Unlock()
		close(handlerDone)
	})
	router.GET("/openai/v1/responses", h.ResponsesWebSocket)
	...
	writeCtx1, cancelWrite1 := context.WithTimeout(context.Background(), 3*time.Second)
	err = clientConn.Write(writeCtx1, coderws.MessageText, []byte(`{"type":"response.create","model":"gpt-5.1","stream":false}`))
	cancelWrite1()
	require.NoError(t, err)

	readCtx1, cancelRead1 := context.WithTimeout(context.Background(), 5*time.Second)
	_, event1, err := clientConn.Read(readCtx1)
	cancelRead1()
	require.NoError(t, err)
	...
	err = clientConn.Write(writeCtx2, coderws.MessageText, []byte(`{"type":"response.create","model":"gpt-5.1","stream":false}`))
	cancelWrite2()
	require.NoError(t, err)

	readCtx2, cancelRead2 := context.WithTimeout(context.Background(), 5*time.Second)
	_, event2, err := clientConn.Read(readCtx2)
	cancelRead2()
	require.NoError(t, err)
	...
	case <-turnHitCh:
	case <-time.After(3 * time.Second):
		t.Fatal("等待上游收到首轮 response.create 超时")
	}
	select {
	case <-turnHitCh:
	case <-time.After(3 * time.Second):
		t.Fatal("等待上游收到第二轮 response.create 超时")
	}
	...
	case <-time.After(5 * time.Second):
		t.Fatal("等待 handler 处理完成超时")
	}

	mu.Lock()
	defer mu.Unlock()
	require.True(t, capturedExists, "expected securityAuditWSTurnContextKey to be set after second-turn BeforeRequest hook fires")
	turnNo, ok := capturedTurn.(int)
	require.True(t, ok, "expected turn value stored in context to be an int")
	require.GreaterOrEqual(t, turnNo, 2)
}
