...
// Keep the classification tests above focused, but exercise the handler's
// actual error-cleanup call site here. The proxy seam returns the same exits
// produced by the relay so the assertions observe scheduler reporting after
// ResponsesWebSocket has accepted a client, parsed its first request and
// selected an account.
func TestOpenAIResponsesWebSocket_ProxyExitAttributionReportsOnlyAccountFailures(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "bare_normal_1000_is_not_reported",
			proxyErr: coderws.CloseError{Code: coderws.StatusNormalClosure, Reason: "client done"},
		},
		{
			name:     "request_cancellation_is_not_reported",
			proxyErr: context.Canceled,
		},
		{
			name: "going_away_without_cancellation_is_reported",
			proxyErr: service.NewOpenAIWSClientCloseError(
				coderws.StatusGoingAway, "upstream going away", errors.New("upstream closed session")),
			wantFailure: true,
		},
		{
			name: "upstream_internal_error_is_reported",
			proxyErr: service.NewOpenAIWSClientCloseError(
				coderws.StatusInternalError, "upstream proxy failed", errors.New("upstream failed")),
			wantFailure: true,
		},
		{
			name:        "upstream_deadline_is_reported",
			proxyErr:    fmt.Errorf("upstream stalled: %w", context.DeadlineExceeded),
			wantFailure: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			case success := <-reports:
				require.True(t, tt.wantFailure, "正常关闭或取消不得报告账号失败")
				require.False(t, success, "这里只应报告失败调度结果")
			default:
				require.False(t, tt.wantFailure, "上游 1001/1011/deadline 必须报告账号失败")
			}
		})
	}
}
