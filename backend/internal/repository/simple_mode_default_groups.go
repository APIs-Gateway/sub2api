...
// backfillSimpleModeGrokDefaultImageGeneration flips allow_image_generation on
// for the auto-created grok-default group if it was created before Grok
// gained image generation support. Only touches rows that still carry the
// auto-created marker (name + description + active), so operator-managed
// groups that deliberately disabled image generation are left untouched.
