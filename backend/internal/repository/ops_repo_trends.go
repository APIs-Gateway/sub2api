...
SELECT
  ...
  -- SLA 失败 = 仅服务可归因(排除 error_owner IN client/client_via_upstream);6 处口径须同步,见 docs/specs/ops-sla-attribution-part-a.md。
  COUNT(*) FILTER (WHERE COALESCE(status_code, 0) >= 400 AND NOT is_business_limited AND COALESCE(error_owner, '') NOT IN ('client', 'client_via_upstream')) AS error_sla,
...
SELECT
  ...
  COUNT(*) AS total,
  -- SLA 失败 = 仅服务可归因(排除 error_owner IN client/client_via_upstream);≥400 在下方外层 WHERE。6 处同步,见 docs/specs/ops-sla-attribution-part-a.md。
  COUNT(*) FILTER (WHERE NOT is_business_limited AND COALESCE(error_owner, '') NOT IN ('client', 'client_via_upstream')) AS sla,
