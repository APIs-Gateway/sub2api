...
// GetByIDForUpdate 同 GetByID 但加行级 FOR UPDATE（不预加载关联，调用方只需卡行本身）。
// 须在事务 context 内调用，锁持有到外层事务结束。
...
func (r *userSubscriptionRepository) translateConditionalWindowReset(ctx context.Context, client *dbent.Client, id int64, affected int, err error) error {
	...
	// A stale reset is an expected no-op. Keep the existing not-found contract
	// for callers that target a row that no longer exists.
