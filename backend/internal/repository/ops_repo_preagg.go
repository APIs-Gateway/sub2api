...
error_agg AS (
  SELECT
    ...
    -- SLA 失败 = 仅服务可归因(排除 error_owner='client')。此口径复制在 6 处,须同步:
    -- ops_repo_preagg.go / ops_repo_dashboard.go / ops_repo_trends.go(x2) / ops_metrics_collector.go / ops_repo.go(列表)。
    -- 详见 docs/specs/ops-sla-attribution-part-a.md。
    COUNT(*) FILTER (WHERE COALESCE(client_status_code, 0) >= 400 AND NOT is_business_limited AND COALESCE(error_owner, '') <> 'client') AS error_count_sla,
