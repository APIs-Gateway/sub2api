...
func (s *UserSubscriptionRepoSuite) TestResetDailyUsage_StaleResetDoesNotClearNewWindowUsage() {
	...
	// A second request still carries the old window snapshot. Its reset must be
	// a no-op instead of clearing usage already recorded in the new window.
	s.Require().NoError(s.repo.ResetDailyUsage(s.ctx, sub.ID, &oldWindowStart, newWindowStart))
...
func (s *UserSubscriptionRepoSuite) TestResetUsageWindows_AdminResetClearsSelectedCurrentWindow() {
	...
	sub := s.mustCreateSubscription(user.ID, group.ID, func(c *dbent.UserSubscriptionCreate) {
		...
		c.SetWeeklyWindowStart(windowStart)
		c.SetMonthlyWindowStart(windowStart)
		c.SetDailyUsageUsd(3)
		c.SetWeeklyUsageUsd(4)
		c.SetMonthlyUsageUsd(5)
	})

	s.Require().NoError(s.repo.ResetUsageWindows(s.ctx, sub.ID, true, false, true, windowStart))

	got, err := s.repo.GetByID(s.ctx, sub.ID)
	s.Require().NoError(err)
	s.Require().Zero(got.DailyUsageUSD)
	s.Require().InDelta(4, got.WeeklyUsageUSD, 1e-6)
