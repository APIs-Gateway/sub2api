...
// ListOpsAccountsForStats loads the minimal account projection needed by the realtime ops dashboard.
...
func (r *accountRepository) ListOAuthRefreshCandidates(ctx context.Context) ([]service.Account, error) {
	...
	// retry-cooldown exclusion below.
	rows, err := r.sql.QueryContext(ctx, `
		SELECT id
		FROM accounts
		WHERE deleted_at IS NULL
...
func (r *accountRepository) RevertProxyFallback(ctx context.Context, accountID int64) error {
	...
	res, err := r.sql.ExecContext(ctx, `
		UPDATE accounts SET
			extra=CASE WHEN platform='openai' AND type='apikey' AND proxy_id IS DISTINCT FROM proxy_fallback_origin_id
