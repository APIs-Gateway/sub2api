...
func TestEnsureSimpleModeDefaultGroups_CreatesMissingDefaults(t *testing.T) {
	...
	grokDefault, err := client.Group.Query().
		...
		Only(seedCtx)
	require.NoError(t, err)
	require.True(t, grokDefault.AllowImageGeneration, "newly auto-created grok default must allow image generation")
}
...
func TestEnsureSimpleModeDefaultGroups_BackfillsOnlyAutoCreatedGrokDefault(t *testing.T) {
	...
	require.NoError(t, err)
	require.True(t, autoDefault.AllowImageGeneration, "pre-existing auto-created grok default must be backfilled")

	operatorGroup, err = client.Group.Get(seedCtx, operatorGroup.ID)
	require.NoError(t, err)
	require.False(t, operatorGroup.AllowImageGeneration, "operator-managed groups must not be touched by the backfill")
}

func TestEnsureSimpleModeDefaultGroups_BackfillPreservesExplicitFalse(t *testing.T) {
	tests := []struct {
		name        string
		description string
		status      string
