...
SELECT
  ...
  COALESCE(COUNT(*) FILTER (WHERE COALESCE(status_code, 0) >= 400 AND NOT is_business_limited AND COALESCE(error_owner, '') NOT IN ('client', 'client_via_upstream')), 0) AS error_sla,
