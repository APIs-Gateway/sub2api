...
func (r *userRepository) Update(ctx context.Context, userIn *service.User) error {
	...
	updateOp := txClient.User.UpdateOneID(userIn.ID).
		...
		SetRpmLimit(userIn.RPMLimit).
		SetStablePriorityEnabled(userIn.StablePriorityEnabled)
...
// ApplyRedeemBalanceAdjustment applies a negative-value redeem code without
// allowing concurrent adjustments to push the balance below zero. Normal usage
// billing deliberately permits overdrafts and must keep using DeductBalance.
func (r *userRepository) ApplyRedeemBalanceAdjustment(ctx context.Context, id int64, delta float64) error {
	...
	result, err := exec.ExecContext(ctx, updateSQL, delta, id)
	if err != nil {
		return err
	}
...
// ApplyRedeemConcurrencyAdjustment applies a negative-value redeem code with
// a database-side floor. It is intentionally separate from UpdateConcurrency,
// which remains the generic additive operation.
