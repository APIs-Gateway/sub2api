...
func (r *usageLogRepository) GetUserModelStats(ctx context.Context, userID int64, startTime, endTime time.Time) (results []ModelStat, err error) {
	return r.getModelStatsWithFiltersBySource(
		ctx, startTime, endTime, userID, 0, 0, 0,
		nil, nil, nil, usagestats.ModelSourceRequested,
	)
}
...
func (r *usageLogRepository) GetStatsWithFilters(ctx context.Context, filters UsageLogFilters) (*UsageStats, error) {
	...
	// 汇总统计 + 三种 endpoint 明细(inbound/upstream/path)原来是 4 次独立扫描,
	// 存在 endpoint 明细不走 model/billing_mode 过滤条件、与汇总口径不一致的问题。
	// 改用一条 GROUPING SETS 查询,让所有维度共享完全相同的 WHERE 过滤条件,一次扫描出全部结果。
	query := fmt.Sprintf(`
		WITH scoped AS (
			SELECT
