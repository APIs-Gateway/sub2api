...
// sortedUniqueAccountIDs 对账号 ID 去重并排序，使 outbox payload 的顺序确定、便于测试与日志比对。
...
func (r *proxyRepository) sweepOneExpiredProxyOnExec(ctx context.Context, client *dbent.Client, exec sqlExecutor, proxyID int64, target *int64, change bool) ([]int64, error) {
	...
	// accountIDs 保持 nil 而非空切片，直到真正扫到一行——调用方（含既有测试）用
	// require.Zero/require.Empty 判断"零改动"，nil 与非 nil 空切片语义不等价。
