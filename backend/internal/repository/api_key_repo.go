...
func (r *apiKeyRepository) Create(ctx context.Context, key *service.APIKey) error {
	builder := r.client.APIKey.Create().
		...
		SetStablePriorityEnabled(key.StablePriorityEnabled)
...
func (r *apiKeyRepository) GetByKeyForAuth(ctx context.Context, key string) (*service.APIKey, error) {
	m, err := r.activeQuery().
		...
		Select(
			...
			apikey.FieldStablePriorityEnabled,
		).
		WithUser(func(q *dbent.UserQuery) {
			q.Select(
				user.FieldID,
...
func (r *apiKeyRepository) Delete(ctx context.Context, id int64) error {
	// 复用调用方事务（若 ctx 携带），保证与 userRepo.Delete 等放进同一外部事务时可原子回滚。
	client := clientFromContext(ctx, r.client)
...
// ListAllByUserID returns filtered keys for an in-memory computed sort.
...
func apiKeyEntityToService(m *dbent.APIKey) *service.APIKey {
	...
	out := &service.APIKey{
		...
		Name:                  m.Name,
		Status:                m.Status,
		StablePriorityEnabled: m.StablePriorityEnabled,
