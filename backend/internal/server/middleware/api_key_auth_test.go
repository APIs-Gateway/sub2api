...
func TestAPIKeyAuthOpenAIQuotaErrorFormat(t *testing.T) {
	...
	cfg := &config.Config{RunMode: config.RunModeStandard}
	router := newAuthTestRouterForPath("/v1/responses", service.NewAPIKeyService(apiKeyRepo, nil, nil, nil, nil, nil, cfg), nil, cfg)
	w := httptest.NewRecorder()
...
func TestAPIKeyAuthQuotaErrorKeepsLegacyFormatOutsideResponses(t *testing.T) {
	...
	cfg := &config.Config{RunMode: config.RunModeStandard}
	router := newAuthTestRouterForPath("/v1/messages", service.NewAPIKeyService(apiKeyRepo, nil, nil, nil, nil, nil, cfg), nil, cfg)
	w := httptest.NewRecorder()
