// Package servertiming provides a request-scoped timing collector that
// renders a standard Server-Timing response header
// (https://www.w3.org/TR/server-timing/). It is currently wired into the
// admin route group only; see internal/server/middleware/server_timing.go.
...
const (
	// HeaderName is the standard Server-Timing response header name.
	HeaderName = "Server-Timing"

	// MetricDatabase and MetricRedis are reserved metric names surfaced as
	// dedicated "db"/"redis" entries in the rendered header, even when no
	// caller has recorded a span for them yet.
	MetricDatabase = "db"
	MetricRedis    = "redis"
