...
func TestNormalizeLenientJSONRequestBody_accepts_client_control_chars_in_strings(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			body:    []byte("{\"input\":\"hello \\\"quoted\\\" \x1b\"}"),
			path:    "input",
			want:    "hello \"quoted\" \x1b",
			wantRaw: `"hello \"quoted\" \u001b"`,
		},
	}
...
func TestNormalizeLenientJSONRequestBody_allows_empty_body(t *testing.T) {
	got, err := NormalizeLenientJSONRequestBody(nil, 1024)
	if err != nil {
		t.Fatalf("NormalizeLenientJSONRequestBody: %v", err)
	}
	if got != nil {
		t.Fatalf("empty body mismatch: got %q", got)
	}
}
...
func TestNormalizeLenientJSONRequestBody_rejects_input_past_limit(t *testing.T) {
	body := []byte("{\"input\":\"hello\"}")

	_, err := NormalizeLenientJSONRequestBody(body, int64(len(body)-1))
	...
	if maxErr.Limit != int64(len(body)-1) {
		t.Fatalf("limit mismatch: got %d want %d", maxErr.Limit, len(body)-1)
	}
}

func TestNormalizeLenientJSONRequestBody_rejects_plain_append_past_limit(t *testing.T) {
	body := []byte("{\"input\":\"\x00a\"}")

	_, err := NormalizeLenientJSONRequestBody(body, 16)
	...
	if maxErr.Limit != 16 {
		t.Fatalf("limit mismatch: got %d want 16", maxErr.Limit)
	}
}

func TestReadLenientJSONRequestBodyWithPrealloc_returns_read_errors(t *testing.T) {
	...
	if err == nil {
		t.Fatal("expected unsupported content encoding error")
	}
}
