...
func TestReleaseAccountSession_ReleasesRegisteredSlot(t *testing.T) {
	...
	svc.ReleaseAccountSession(context.Background(), newSessionLimitTestAccount(), "session-hash-1")
...
func TestReleaseAccountSession_NoOpForInapplicableAccounts(t *testing.T) {
	apiKeyAcc := &Account{ID: 43, Platform: PlatformAnthropic, Type: AccountTypeAPIKey, Extra: map[string]any{"max_sessions": 1}}
	noLimitAcc := &Account{ID: 44, Platform: PlatformAnthropic, Type: AccountTypeOAuth}
...
func TestReleaseAccountSession_Idempotent(t *testing.T) {
	...
	svc.ReleaseAccountSession(context.Background(), account, "session-hash")
	svc.ReleaseAccountSession(context.Background(), account, "session-hash")
