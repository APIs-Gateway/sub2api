...
func (r *updatingProxyRepoStub) GetByID(context.Context, int64) (*Proxy, error) {
	return r.proxy, nil
}

func (r *updatingProxyRepoStub) Update(_ context.Context, proxy *Proxy) error {
	r.updateCalls++
	r.proxy = proxy
	return nil
}
...
func TestAdminProxyUpdateCredentials(t *testing.T) {
	check := func(name string, input UpdateProxyInput, username, password string) {
		...
		t.Run(name, func(t *testing.T) {
			repo := &updatingProxyRepoStub{proxy: &Proxy{
