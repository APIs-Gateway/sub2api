...
// prepareCodexAccountIdentitySource stages the selected account once per
// attempt. The handler reuses gin.Context across failover attempts, so every
...
func codexAccountIdentityNamespace(account *Account) string {
	if account == nil || !account.IsOpenAI() || (account.Type != AccountTypeOAuth && account.Type != AccountTypeSetupToken) {
		return ""
	}
