...
// openAIStreamEventIsTerminalWithType reuses a type extracted by the caller
...
// hasObservedTokens 报告流式转发过程中是否已观测到任何上游计量 token。
...
// partialStreamUsageResult 在流式转发中途出错时，把已观测到的 usage 部分结果包装为
// ForwardResult（与错误一起返回给 handler 记录）。上游一旦下发过 message_start，
// input/cache token 就已计量，直接丢弃会让请求完全漏计流式计费（issue #764）。
// 无已观测 usage 时返回 nil。
//
// 不变式：UpstreamFailoverError 必须保持 result=nil——failover 重试成功后按成功请求
// 计费，若同时返回部分 usage 会造成双重记账，此处显式兜底防御。
	...
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		...
		if resp.StatusCode >= 400 && resp.StatusCode != 400 && s.shouldRetryUpstreamError(account, resp.StatusCode) {
			if attempt < maxRetryAttempts {
				...
				_ = resp.Body.Close()
				s.handleRetryable429SideEffects(ctx, account, resp.StatusCode, resp.Header, respBody, reqModel)
				appendOpsUpstreamError(c, OpsUpstreamErrorEvent{
					Platform:           account.Platform,
					AccountID:          account.ID,
					AccountName:        account.Name,
	...
	if resp.StatusCode >= 400 && s.shouldRetryUpstreamError(account, resp.StatusCode) {
		if s.shouldFailoverUpstreamError(resp.StatusCode) {
			...
			logger.LegacyPrintf("service.gateway", "[Forward] Upstream error (retry exhausted, failover): Account=%d(%s) Status=%d RequestID=%s Body=%s",
				account.ID, account.Name, resp.StatusCode, upstreamRequestIDFromHeader(resp.Header), truncateString(string(respBody), 1000))
		...
		if err != nil {
			...
			if errors.As(err, &sseErr) {
				...
				logger.LegacyPrintf("service.gateway",
					...
					account.ID, account.Name, upstreamRequestIDFromHeader(resp.Header),
			...
			// usage 与错误一起返回，handler 在错误处理完成后照常提交 usage 记录（#764）。
...
func (s *GatewayService) forwardAnthropicAPIKeyPassthroughWithInput(
	...
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		...
		if resp.StatusCode >= 400 && resp.StatusCode != 400 && s.shouldRetryUpstreamError(account, resp.StatusCode) {
			if attempt < maxRetryAttempts {
				...
				respBody, _ := s.readUpstreamErrorBody(resp)
				_ = resp.Body.Close()
				s.handleRetryable429SideEffects(ctx, account, resp.StatusCode, resp.Header, respBody, input.RequestModel)
				appendOpsUpstreamError(c, OpsUpstreamErrorEvent{
					Platform:           account.Platform,
					AccountID:          account.ID,
					AccountName:        account.Name,
	...
	if input.RequestStream {
		...
		if err != nil {
			// 流中断时保留已探测到的 usage 与错误一起返回，避免上游已产生消耗的
			// 透传请求完全漏计流式计费（#764）。
...
func (s *GatewayService) invalidNonStreamingJSONFailoverError(
	...
	logger.LegacyPrintf(
		...
		accountName,
		resp.StatusCode,
		upstreamRequestIDFromHeader(resp.Header),
		parseErr,
	)
...
func (s *GatewayService) handleNonStreamingResponseAnthropicAPIKeyPassthrough(
	...
	body, err = s.classifyAnthropicPassthroughResponse(ctx, body, usage)
	if err != nil {
		return nil, err
	}
...
func (s *GatewayService) classifyAnthropicPassthroughResponse(ctx context.Context, body []byte, usage *ClaudeUsage) ([]byte, error) {
	if !IsForceCacheBilling(ctx) || usage == nil || usage.InputTokens <= 0 {
		return body, nil
	}
	return classifyAnthropicResponseInputAsCacheRead(body, usage)
}

func classifyAnthropicResponseInputAsCacheRead(body []byte, usage *ClaudeUsage) ([]byte, error) {
	if !gjson.ValidBytes(body) {
		return nil, fmt.Errorf("classify forced cache billing response: invalid JSON")
	}
	...
	return sjson.SetBytes(classified, "usage.cache_read_input_tokens", usage.CacheReadInputTokens+usage.InputTokens)
}
	...
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		...
		if resp.StatusCode >= 400 && resp.StatusCode != 400 && s.shouldRetryUpstreamError(account, resp.StatusCode) {
			if attempt < maxRetryAttempts {
				...
				respBody, _ := s.readUpstreamErrorBody(resp)
				_ = resp.Body.Close()
				s.handleRetryable429SideEffects(ctx, account, resp.StatusCode, resp.Header, respBody)
				appendOpsUpstreamError(c, OpsUpstreamErrorEvent{
					Platform:           account.Platform,
					AccountID:          account.ID,
					AccountName:        account.Name,
...
func (s *GatewayService) handleErrorResponse(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, requestedModel ...string) (*ForwardResult, error) {
	...
	logger.LegacyPrintf("service.gateway", "[Forward] Upstream error (non-retryable): Account=%d(%s) Status=%d RequestID=%s Body=%s",
		account.ID, account.Name, resp.StatusCode, upstreamRequestIDFromHeader(resp.Header), truncateString(string(body), 1000))
...
func (s *GatewayService) handleRetryExhaustedSideEffects(ctx context.Context, resp *http.Response, account *Account) {
	...
	if statusCode == http.StatusTooManyRequests {
		s.handleRetryable429SideEffects(ctx, account, statusCode, resp.Header, body)
		logger.LegacyPrintf("service.gateway", "Account %d: recorded 429 after %d retries", account.ID, maxRetryAttempts)
		return
	}
...
func (s *GatewayService) handleRetryable429SideEffects(ctx context.Context, account *Account, statusCode int, headers http.Header, body []byte, requestedModel ...string) {
	if s == nil || s.rateLimitService == nil || account == nil || statusCode != http.StatusTooManyRequests {
		return
	}
	if len(requestedModel) > 0 {
		s.rateLimitService.HandleUpstreamError(ctx, account, statusCode, headers, body, requestedModel[0])
		return
	}
...
// ResolveUserGroupRateMultiplier exposes the same cached multiplier used by usage billing.
...
func writeUsageLogBestEffort(ctx context.Context, repo UsageLogRepository, usageLog *UsageLog, logKey string) {
	...
	if writer, ok := repo.(usageLogBestEffortWriter); ok {
		if err := writer.CreateBestEffort(usageCtx, usageLog); err != nil {
			...
			// 计费已完成；队列超时标记为 dropped 时也必须同步兜底，依赖 request_id 幂等。
			fallbackCtx := usageCtx
			if usageCtx.Err() != nil {
				// 入队阻塞可能耗尽 detached 窗口；换一个独立窗口，避免兜底必然失败。
