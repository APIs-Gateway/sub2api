...
type openAIRawStreamTerminalState struct {
	...
	// 截断判定时 clientOutputStarted 仍为 false，需要单独识别以保持原样透传。
	sawNonSSEBody   bool
	sawDone         bool
	sawUsage        bool
...
// ObserveNonDataLine 记录非 data: 行。空行、SSE 注释（": ..."）与 event/id/retry
// 字段行属于 SSE 语义本身，不算非 SSE 响应体。
func (t *openAIRawStreamTerminalState) ObserveNonDataLine(line string) {
	if t == nil || t.sawNonSSEBody {
		return
	}
...
// IsTruncated 判定上游是否在任何终止信号之前结束。clientOutputStarted 与
// sawNonSSEBody 用于放行非 SSE 响应体：那类响应本就没有 data: 行，既有行为是原样
// 透传，不在本次判定范围内；但"一个字节都没收到"（或只有注释/心跳）的 200 依然算截断。
func (t *openAIRawStreamTerminalState) IsTruncated(clientOutputStarted bool) bool {
	if t == nil || t.Terminated() {
		return false
	}
	return t.sawDataLine || (!clientOutputStarted && !t.sawNonSSEBody)
}
