...
func (s *GeminiMessagesCompatService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*ForwardResult, error) {
	...
	if resp.StatusCode >= 400 {
		...
		if s.shouldFailoverGeminiUpstreamError(account, resp.StatusCode) {
			upstreamReqID := resp.Header.Get(requestIDHeader)
			if upstreamReqID == "" {
				upstreamReqID = resp.Header.Get("x-goog-request-id")
			}
...
func (s *GeminiMessagesCompatService) shouldFailoverGeminiUpstreamError(account *Account, statusCode int) bool {
	switch statusCode {
	case 401, 403, 529:
		return true
...
func (s *GeminiMessagesCompatService) ForwardAIStudioGET(ctx context.Context, account *Account, path string) (*UpstreamHTTPResult, error) {
	...
	// 见 upstream_path_guard.go。upstream sync (#5137)。
...
// incrementIntegralSchemaBound 将 JSON Schema 的 exclusiveMinimum 换算为等价的
// 整数 minimum（x > N 等价于 x >= N+1）。仅当输入是一个有限、无溢出风险的整数值时才转换，
// 否则返回 false，调用方会直接丢弃该约束而不是生成错误的边界。
...
// schemaNumberLess 比较两个可能类型不同的 JSON Schema 数值字段，用于判断是否需要
// 用换算出的 minimum 覆盖已存在的 minimum（只在换算值更严格时才覆盖）。
