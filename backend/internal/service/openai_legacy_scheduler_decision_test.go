	...
	for _, loadBatch := range []bool{true, false} {
		...
		t.Run(name, func(t *testing.T) {
			...
			selection, decision, err := svc.SelectAccountWithSchedulerForCapability(
				...
				OpenAIUpstreamTransportAny, OpenAIEndpointCapabilityChatCompletions, false, true,
			)
...
func TestLegacySchedulerDecision_PreviousResponseRouting(t *testing.T) {
	...
	t.Run("routes to the account owning the response", func(t *testing.T) {
		...
		selection, decision, err := svc.SelectAccountWithSchedulerForCapability(
			...
			OpenAIUpstreamTransportResponsesWebsocketV2, OpenAIEndpointCapabilityChatCompletions, false, true,
		)
		require.NoError(t, err)
		require.NotNil(t, selection)
		require.NotNil(t, selection.Account)
	...
	// fork 的 selectAccountByPreviousResponseIDForCapability 在抢槽前就按 WSv2 传输过滤持有账号，
	// 因此这里断言回退结果，并要求持有账号若被抢过槽则一定被释放（不泄漏槽位）。
	t.Run("transport-incompatible owner falls back without leaking its slot", func(t *testing.T) {
		acquired := make([]int64, 0)
		released := make([]int64, 0)
		svc := newLegacySchedulerDecisionTestService(newLegacySchedulerDecisionTestAccounts(groupID, false), true, schedulerTestConcurrencyCache{acquiredIDs: &acquired, releasedIDs: &released})
		require.NoError(t, svc.getOpenAIWSStateStore().BindResponseAccount(ctx, groupID, responseID, 38102, time.Hour))
		...
		require.Equal(t, openAIAccountScheduleLayerLoadBalance, decision.Layer)
		require.False(t, decision.StickyPreviousHit)
		require.Equal(t, countLegacySchedulerDecisionIDs(acquired, 38102), countLegacySchedulerDecisionIDs(released, 38102),
			"every slot acquired on the transport-incompatible owner must be released")
	})
...
func countLegacySchedulerDecisionIDs(ids []int64, target int64) int {
	n := 0
	for _, id := range ids {
		if id == target {
			n++
