...
// 同一种子永远返回同一值。仅用于探测流量身份，不涉及设备指纹收敛
// （fork 尚未引入该特性，参见 upstream #5553，本文件不依赖它）。
...
// session-id / thread-id 恒为 UUID，探测既然与真实流量走同一个 /responses
// 端点，标识形态就必须同构——否则上游能凭 "probe_compact_5" 这类字面量一眼
// 区分出探测流量。账号级稳定派生：重复探测复用同一会话，而不是每次新开一个。
