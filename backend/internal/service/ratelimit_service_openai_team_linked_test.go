...
// newTeamLinkedFixture: #1 触发者(team-A) #2 同队 #3 异队 #4 apikey #6 同队 #7 同队但已 error
//
// 上游原始 fixture 还包含一个 #5 影子账号（ParentAccountID 非空），用于验证
// fan-out 会跳过影子账号；fork 的 Account 模型没有影子账号概念（ParentAccountID/
// IsShadow，见 openai_team_linked_error.go 对应说明），因此不构造该用例，ID 5 留空。
