...
// 各普通平台严格独立；composite 分组为每个 concrete provider 建立独立索引。
...
// 各普通平台严格独立；composite 分组允许 concrete provider 定价进入缓存。
...
// 普通分组严格只返回自身，composite 分组展开为 concrete providers。
...
func (s *ChannelService) lookupGroupChannel(ctx context.Context, groupID int64) (*channelLookup, error) {
	...
	if resolvedPlatform, ok := ResolvedTargetPlatformFromContext(ctx); ok && isConcreteRequestPlatform(resolvedPlatform) {
			...
			if conflictsBetween(entries[i], entries[j]) {
				return infraerrors.BadRequest(errCode,
					fmt.Sprintf("%s '%s' and '%s' conflict in platform '%s': overlapping match range "+
						"(model names are matched case-insensitively, so an existing entry covers all case variants)",
