...
// redeemRejectRepo serves a single redeem code; any other code is not found.
// (Fork adaptation: upstream defines this helper in a test file the fork
// does not have.)
...
func TestSuccessfulPublicRedeemDoesNotMutateFailureCounter(t *testing.T) {
	...
	redeemRepo := &redeemIsolationRepo{
		redeemCodeRepoStub: redeemCodeRepoStub{codesByCode: map[string]*RedeemCode{code.Code: code}},
	}
