...
func TestSchedulerSnapshotServiceTriggerFullRebuildRequiresGroupLifecycleAuthority(t *testing.T) {
	cache := &schedulerFullRebuildTestCache{}
	svc := NewSchedulerSnapshotService(cache, nil, nil, nil, nil)

	require.ErrorIs(t, svc.triggerFullRebuild("manual"), ErrSchedulerCacheNotReady)
	cache.mu.Lock()
	listCalls := cache.listCalls
	captures := cache.captures
	lockCalls := cache.lockCalls
