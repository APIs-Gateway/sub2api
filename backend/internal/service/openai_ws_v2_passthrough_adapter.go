...
func (c *openAIWSPassthroughFirstOutputFrameConn) ReadFrame(ctx context.Context) (coderws.MessageType, []byte, error) {
	...
	resetTimer := func() {
		...
		if c.now != nil {
			remaining = state.deadline.startedAt.Add(state.deadline.timeout).Sub(nowFn())
		}
		if remaining < 0 {
			remaining = 0
		}
	...
	for {
		...
		case <-timerCh:
			...
			if c.now != nil && nowFn().Before(state.deadline.startedAt.Add(state.deadline.timeout)) {
				resetTimer()
				continue
			}
			cancelRead()
...
func (s *OpenAIGatewayService) proxyResponsesWebSocketV2Passthrough(
	...
	for {
		...
		if s.isAgentIdentityAccount(ctx, account) && isAgentIdentityTaskInvalidWSDialError(dialErr) && !agentTaskRecoveryTried {
			...
			if refreshErr != nil {
				return fmt.Errorf("refresh agent identity authentication failed: %w", refreshErr)
			}
	...
	relayResult, relayExit := openaiwsv2.RunEntry(openaiwsv2.EntryInput{
		...
		Options: openaiwsv2.RelayOptions{
			...
			// Active turns are bounded by relayUpstreamFrameConn; the
			// client reader owns the post-terminal idle window.
			IdleTimeout:                     0,
...
func openAIWSPassthroughNormalizeRelayError(
	...
	if errors.As(relayErr, &firstOutputTimeoutErr) {
		if completedTurns > 0 || wroteDownstream {
			...
			relayErr = &UpstreamFailoverError{
