...
// upstream(1c214eaca4f4): Grok OAuth accounts must never be routed through the
// ChatGPT Codex identity-restoration path added for OpenAI OAuth accounts;
// client-supplied User-Agent/OpenAI-Beta must pass through unchanged and no
// originator/version should be injected.
func TestOpenAIGatewayServiceGrokForwardAsAnthropicKeepsOwnIdentity(t *testing.T) {
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
