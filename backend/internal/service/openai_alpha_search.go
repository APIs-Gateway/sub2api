...
func (s *OpenAIGatewayService) ForwardAlphaSearch(ctx context.Context, c *gin.Context, account *Account, body []byte) error {
	...
	if err != nil {
		return fmt.Errorf("sanitize alpha search request body: %w", err)
	}
	body = sanitizedBody
	...
	if resp.StatusCode >= http.StatusBadRequest {
		...
		if s.shouldFailoverOpenAIUpstreamResponse(account, resp.StatusCode, upstreamMessage, respBody) || endpointUnsupported {
...
func (s *OpenAIGatewayService) forwardAlphaSearchViaResponsesWebSearch(
	...
	respBody, err := ReadUpstreamResponseBody(resp.Body, s.cfg, c, openAITooLargeError)
	if err != nil {
		return fmt.Errorf("read alpha search responses fallback response: %w", err)
	}
