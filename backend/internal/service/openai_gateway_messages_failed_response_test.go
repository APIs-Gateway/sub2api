...
func TestForwardAsAnthropic_BufferedResponseFailed_ReturnsError(t *testing.T) {
	...
	require.Equal(t, "Content policy violation", events[0].Message)
...
func TestForwardAsAnthropic_StreamingResponseFailed_Failover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	require.Error(t, err)
	var failoverErr *UpstreamFailoverError
	require.True(t, errors.As(err, &failoverErr), "rate_limit_error should trigger streaming UpstreamFailoverError for failover, got: %T: %v", err, err)
}
