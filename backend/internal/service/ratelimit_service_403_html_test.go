...
// countingOpenAI403CounterCache 在既有桩 openAI403CounterCacheStub 的基础上记录
// 递增调用次数。HTML 403 不仅不能处罚账号，连计数都不能加——否则后续真实的
// 账号级 403 会踩着这些"白涨"的计数提前触发永久禁用。
...
type openAI403HTMLTestHarness struct {
...
func newOpenAI403HTMLTestHarness(t *testing.T, accountID int64, counts ...int64) *openAI403HTMLTestHarness {
	t.Helper()
	repo := &rateLimitAccountRepoStub{}
	...
	return &openAI403HTMLTestHarness{
		svc:     svc,
		repo:    repo,
		counter: counter,
		blocker: blocker,
...
func (h *openAI403HTMLTestHarness) handle(body string) bool {
	return h.svc.HandleUpstreamError(
		context.Background(), h.account, http.StatusForbidden, http.Header{}, []byte(body),
	)
}

func (h *openAI403HTMLTestHarness) requireNoAccountPenalty(t *testing.T) {
	t.Helper()
...
// 无效子路径被转发后，上游代理在到达 OpenAI API 之前回 HTML 403 页面。这是
// 链路/端点级响应，不是账号凭据失效的证据。
...
func TestHandleUpstreamError_OpenAIHTML403DoesNotPenalizeAccount(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			h := newOpenAI403HTMLTestHarness(t, 601, 1)
...
func TestHandleUpstreamError_OpenAIHTML403RepeatedNeverEscalates(t *testing.T) {
	h := newOpenAI403HTMLTestHarness(t, 602, 1, 2, 3, 4, 5)
...
func TestHandleUpstreamError_OpenAIStructured403StillPenalizes(t *testing.T) {
	t.Run("first_hit_temp_unschedulable", func(t *testing.T) {
		h := newOpenAI403HTMLTestHarness(t, 603, 1)
	...
	t.Run("threshold_disables", func(t *testing.T) {
		h := newOpenAI403HTMLTestHarness(t, 604, int64(openAI403DisableThreshold))
	...
	t.Run("plain_text_body_unchanged", func(t *testing.T) {
		h := newOpenAI403HTMLTestHarness(t, 605, 1)
...
func TestHandleUpstreamError_HTML403OnOtherPlatformsUnchanged(t *testing.T) {
	for _, platform := range []string{PlatformAnthropic, PlatformGemini} {
		t.Run(platform, func(t *testing.T) {
			repo := &rateLimitAccountRepoStub{}
			svc := NewRateLimitService(repo, nil, &config.Config{}, nil, nil)
			account := &Account{ID: 606, Platform: platform, Type: AccountTypeAPIKey}
