...
func TestDownstreamRejectsSSECommentsReadsBothHeaders(t *testing.T) {
	...
	c, _ := newGeminiSSECompatTestContext(http.MethodPost, "/v1beta/models/gemini-3.8-flash:streamGenerateContent")
...
func runAntigravityGeminiStreamWithIdle(t *testing.T, userAgent string, idle time.Duration) string {
	t.Helper()
	gin.SetMode(gin.TestMode)
	svc := newGeminiSSECompatTestService(config.GatewayConfig{MaxLineSize: defaultMaxLineSize, StreamKeepaliveInterval: 1})
	c, recorder := newGeminiSSECompatTestContext(http.MethodPost, "/v1beta/models/gemini-3.8-flash:streamGenerateContent")
	if userAgent != "" {
...
// newGeminiSSECompatTestService / newGeminiSSECompatTestContext 是 fork 适配：上游测试用的
// newAntigravityCompatService / newAntigravityCompatContext 在 fork 中不存在，这里用 fork
// 现有的构造方式最小化构造，名字刻意区分以免将来同步上游 compat 测试文件时重名。
func newGeminiSSECompatTestService(cfg config.GatewayConfig) *AntigravityGatewayService {
	return &AntigravityGatewayService{
		settingService: NewSettingService(&antigravitySettingRepoStub{}, &config.Config{Gateway: cfg}),
	}
}

func newGeminiSSECompatTestContext(method, path string) (*gin.Context, *httptest.ResponseRecorder) {
