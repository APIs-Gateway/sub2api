...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_TurnRetryForcesFreshConn(t *testing.T) {
	...
	// Add a second stale idle connection. The fork has no handshake-affinity
	// fields, so admission into this account pool is sufficient to reproduce a
	// retry that must create a fresh connection rather than reusing either one.
