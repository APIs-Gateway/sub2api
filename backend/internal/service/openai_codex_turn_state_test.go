...
func TestOpenAICodexClientSessionIDHeader(t *testing.T) {
	require.Empty(t, openAICodexClientSessionIDHeader(nil))

	h := http.Header{}
	require.Empty(t, openAICodexClientSessionIDHeader(h))

	h.Set("session_id", "underscore-form")
	require.Equal(t, "underscore-form", openAICodexClientSessionIDHeader(h))

	// 连字符形式优先于下划线形式
	h.Set("session-id", "hyphen-form")
	require.Equal(t, "hyphen-form", openAICodexClientSessionIDHeader(h))
}
...
func TestRelayOpenAICodexTurnState_NoopWithoutWriter(t *testing.T) {
	svc := &OpenAIGatewayService{}
	svc.relayOpenAICodexTurnState(nil, &Account{ID: 1}, http.Header{})
	svc.relayOpenAICodexTurnState(&gin.Context{}, &Account{ID: 1}, http.Header{})
}
...
func TestStageOpenAICodexTurnState_NilDstNoop(t *testing.T) {
	upstream := http.Header{}
	upstream.Set("x-codex-turn-state", "blob-C")
	stageOpenAICodexTurnState(nil, upstream)
}
...
func TestNoteOpenAICodexTurnStateProvenance_GuardClauses(t *testing.T) {
	var nilSvc *OpenAIGatewayService
	nilSvc.noteOpenAICodexTurnStateProvenance(nil, &Account{ID: 1})

	svc := &OpenAIGatewayService{}
	c, _ := newTurnStateTestContext(t, 1, "sess-guard")
	svc.noteOpenAICodexTurnStateProvenance(c, nil)
	svc.noteOpenAICodexTurnStateProvenance(c, &Account{ID: 0})

	_, ok := svc.openaiCodexTurnStateOrigins.Load("1\x00sess-guard")
	require.False(t, ok)
}
...
func TestGuardOpenAICodexTurnStateEcho(t *testing.T) {
	...
	t.Run("corrupted_origin_entry_is_deleted", func(t *testing.T) {
		svc := &OpenAIGatewayService{}
		c, _ := newTurnStateTestContext(t, 7, "sess-g6")
		svc.openaiCodexTurnStateOrigins.Store("7\x00sess-g6", "not-an-origin")
		h := newOutbound("blob-A")
		svc.guardOpenAICodexTurnStateEcho(c, &Account{ID: 43}, h)
		require.Equal(t, "blob-A", h.Get("x-codex-turn-state"))
		_, ok := svc.openaiCodexTurnStateOrigins.Load("7\x00sess-g6")
		require.False(t, ok)
	})

	t.Run("guard_clauses", func(t *testing.T) {
		var nilSvc *OpenAIGatewayService
		nilSvc.guardOpenAICodexTurnStateEcho(nil, nil, nil)

		svc := &OpenAIGatewayService{}
		svc.guardOpenAICodexTurnStateEcho(nil, &Account{ID: 1}, nil)
		svc.guardOpenAICodexTurnStateEcho(nil, nil, http.Header{})
	})
}
...
func TestSweepOpenAICodexTurnStateOrigins_PrunesExpiredEntries(t *testing.T) {
	...
	svc.openaiCodexTurnStateOrigins.Store("corrupted", "not-an-origin")
	...
	_, corruptedOK := svc.openaiCodexTurnStateOrigins.Load("corrupted")
	require.False(t, corruptedOK)
}

func TestSweepOpenAICodexTurnStateOrigins_SkipsUntilBoundary(t *testing.T) {
	svc := &OpenAIGatewayService{}
	svc.openaiCodexTurnStateOrigins.Store("expired", openAICodexTurnStateOrigin{
		accountID: 1,
		expiresAt: time.Now().Add(-time.Minute),
	})
	svc.openaiCodexTurnStateWrites.Store(1)
	svc.sweepOpenAICodexTurnStateOrigins()

	// 未到 256 的倍数：不清扫
	_, ok := svc.openaiCodexTurnStateOrigins.Load("expired")
	require.True(t, ok)
}

func TestExtractOpenAICodexTurnState(t *testing.T) {
	require.Empty(t, extractOpenAICodexTurnState(nil))
	require.Empty(t, extractOpenAICodexTurnState(http.Header{}))

	h := http.Header{}
	h.Set("x-codex-turn-state", "  blob-trim  ")
	require.Equal(t, "blob-trim", extractOpenAICodexTurnState(h))
}
