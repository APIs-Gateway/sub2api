...
func (s *raceSafeUserRepo) userCount() int {
	s.mu.Lock()
	defer s.mu.Unlock()
	return len(s.byID)
}
...
func invitationRaceSettings() map[string]string {
...
// 兜底并硬删败者账号），并发下仅最先占码的注册成功，且不留下孤儿账号。
func TestAuthService_Register_InvitationCodeSingleUseUnderConcurrency(t *testing.T) {
	...
	require.Equal(t, 1, userRepo.userCount(), "被拒绝的并发注册不得留下账号")
	...
	_, err = userRepo.GetByID(ctx, *claimed.UsedBy)
	require.NoError(t, err, "used_by 指向的必须是留存下来的那个账号")
}
...
func TestAuthService_Register_InvitationCodeRejectedWhenAlreadyUsed(t *testing.T) {
	...
	require.Zero(t, userRepo.userCount())
}
...
// TestAuthService_Register_InvitationClaimLostBetweenCheckAndClaim 覆盖“检查通过、
// 占用失败”这一窗口：GetByCode 时还是 unused，到占用时已被别人抢走。注册必须以
// INVITATION_CODE_INVALID 失败，并把刚建出的账号硬删掉（退化路径下的补偿）。
func TestAuthService_Register_InvitationClaimLostBetweenCheckAndClaim(t *testing.T) {
	const code = "INV-RACE-003"
	userRepo := newRaceSafeUserRepo()
	redeemRepo := &stolenOnUseRedeemRepo{raceSafeRedeemRepo: raceSafeRedeemRepo{codes: map[string]*RedeemCode{
		code: {ID: 3, Code: code, Type: RedeemTypeInvitation, Status: StatusUnused},
	}}}
	svc := newOAuthEmailFlowAuthService(
		userRepo,
		redeemRepo,
	...
	_, _, err := svc.RegisterWithVerification(context.Background(), "loser@example.com", "Password123!", "", "", code, "")
	require.ErrorIs(t, err, ErrInvitationCodeInvalid)
	require.Zero(t, userRepo.userCount(), "占码失败时不得留下账号")
	require.Equal(t, []int64{1}, userRepo.deleted, "必须回滚刚建出的账号")
}

// stolenOnUseRedeemRepo 在 Use 被调用前一刻把码标记为已被他人占用，模拟 TOCTOU 窗口。
type stolenOnUseRedeemRepo struct {
	raceSafeRedeemRepo
}

func (s *stolenOnUseRedeemRepo) Use(ctx context.Context, id, userID int64) error {
	s.mu.Lock()
	other := int64(999)
	for _, c := range s.codes {
		if c.ID == id {
