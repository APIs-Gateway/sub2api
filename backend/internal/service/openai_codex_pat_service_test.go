...
func TestOpenAIOAuthService_ValidateCodexPersonalAccessTokenRequiresATPrefix(t *testing.T) {
	...
	_, err := svc.ValidateCodexPersonalAccessToken(context.Background(), "", "")
	require.Error(t, err)
...
func TestOpenAIOAuthService_ValidateCodexPersonalAccessTokenRejectsInvalidResponses(t *testing.T) {
	var statusCode int
	...
	tests := []struct {
		...
		{name: "unauthorized", statusCode: http.StatusUnauthorized, body: "invalid", want: "invalid or expired"},
		{name: "forbidden", statusCode: http.StatusForbidden, body: "invalid", want: "invalid or expired"},
		{name: "upstream error", statusCode: http.StatusBadGateway, body: "upstream unavailable", want: "upstream unavailable"},
		{name: "malformed json", statusCode: http.StatusOK, body: `{invalid`, want: "invalid Codex personal access token validation response"},
		{name: "missing metadata", statusCode: http.StatusOK, body: `{"email":"user@example.com"}`, want: "missing"},
	}
