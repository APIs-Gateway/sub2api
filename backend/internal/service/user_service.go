...
type UserRepository interface {
	...
	// SetSubscriptionOverdraftGuard 设置用户的订阅透支闸门触发位。
	SetSubscriptionOverdraftGuard(ctx context.Context, id int64, enabled bool) error
...
// RedeemUserAdjustmentRepository provides atomic floor-at-zero updates for
// negative redeem codes. It stays narrower than UserRepository because normal
// usage billing is intentionally allowed to overdraw.
...
func (s *UserService) UpdateProfile(ctx context.Context, userID int64, req UpdateProfileRequest) (*User, error) {
	if txRunner, ok := s.userRepo.(userProfileIdentityTxRunner); ok {
		...
		if s.authCacheInvalidator != nil && updated != nil && (updated.Concurrency != oldConcurrency || req.StablePriorityEnabled != nil) {
			s.authCacheInvalidator.InvalidateAuthCacheByUserID(ctx, userID)
		}
		return updated, nil
	}
	...
	if s.authCacheInvalidator != nil && (updated.Concurrency != oldConcurrency || req.StablePriorityEnabled != nil) {
		s.authCacheInvalidator.InvalidateAuthCacheByUserID(ctx, userID)
	}
	return updated, nil
}
...
func (s *UserService) updateProfile(ctx context.Context, userID int64, req UpdateProfileRequest) (*User, int, error) {
	...
	if req.StablePriorityEnabled != nil {
		user.StablePriorityEnabled = *req.StablePriorityEnabled
	}
...
func (s *UserService) sendNotifyVerifyEmail(ctx context.Context, emailService *EmailService, userID int64, email, code, locale string) error {
	...
	resolvedLocale := emailService.resolveLegacyEmailLocale(ctx, userID, email, locale)
	subject := notifyVerifyEmailSubject(siteName, resolvedLocale)
	body := buildNotifyVerifyEmailBodyForLocale(code, siteName, resolvedLocale)
...
func notifyVerifyEmailSubject(siteName, locale string) string {
	if isNotificationChineseLocale(locale) {
		return localizeChineseNotificationEmailText(locale, fmt.Sprintf("[%s] 通知邮箱验证码", siteName))
	}
	return fmt.Sprintf("[%s] Notification Email Verification", siteName)
}

func buildNotifyVerifyEmailBodyForLocale(code, siteName, locale string) string {
	if isNotificationChineseLocale(locale) {
		return localizeChineseNotificationEmailText(locale, fmt.Sprintf(`<!DOCTYPE html><html><body><h1>%s</h1><p>通知邮箱验证码</p><div style="font-size:36px;font-weight:bold;letter-spacing:8px;">%s</div><p>您正在添加额外的通知邮箱，请输入此验证码完成验证。</p><p>此验证码将在 <strong>15 分钟</strong>后失效。</p><p>如果您没有请求此验证码，请忽略此邮件。</p></body></html>`, siteName, code))
	}
	return fmt.Sprintf(`<!DOCTYPE html><html><body><h1>%s</h1><p>Notification Email Verification</p><div style="font-size:36px;font-weight:bold;letter-spacing:8px;">%s</div><p>You are adding an extra notification email. Please enter this code to verify.</p><p>This code will expire in <strong>15 minutes</strong>.</p><p>If you did not request this code, please ignore this email.</p></body></html>`, siteName, code)
}
