...
// MarkOpenAICompactClientStream records the original streaming intent for a
// body-signalled compact request before compact normalization removes stream.
...
// writeOpenAICompactSSEBridge converts a unary compact JSON response into the
// minimal Responses SSE sequence expected by body-signalled Codex clients.
// Callers retain their existing response path whenever this returns false.
...
func writeOpenAICompactSSEFailureMessage(c *gin.Context, statusCode int, errType, message string) {
	...
	payload, err := json.Marshal(map[string]any{
		...
		"sequence_number": NextResponsesStreamSequence(c),
...
func buildOpenAICompactSSEPayload(finalResponse []byte) ([]byte, bool) {
	...
	// SSE data fields must be one physical line, including when upstream sends
	// pretty-printed JSON.
