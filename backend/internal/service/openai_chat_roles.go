...
// requiresSystemChatRole reports whether the selected API-key destination is
// a strict Chat Completions provider that rejects the "developer" role
// (DeepSeek, Kimi/Moonshot, Zhipu/Z.ai). The fork has no dedicated platforms
// for these providers; they are reached through OpenAI-compatible API-key
// accounts, so match the selected destination's exact hostname, never the
// requested model ID.
