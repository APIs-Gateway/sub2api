...
// UpstreamBillingProbeMaxBatchSize bounds both manual batches and one runner cycle.
...
// UpstreamBillingProbeResult is returned by a bounded manual batch.
...
// SetUpstreamBillingProbeSettings validates and persists runner settings.
...
// UpstreamBillingProbeService discovers and persists a remote Sub2API billing snapshot.
// The service is intentionally not wired into the server in this slice; the admin API
// slice will own lifecycle registration after this producer contract is reviewed.
...
// ProvideUpstreamBillingProbeService is the lifecycle hook used by the later wire slice.
...
func (s *UpstreamBillingProbeService) RunDue(ctx context.Context) error {
	...
	lockNow := s.currentTime().UTC()
	runRelease, acquired := tryAcquireSingletonLeaderLock(ctx, s.lockCache, s.db, upstreamBillingProbeLeaderLockKey, s.instanceID, upstreamBillingProbeLeaderLockTTL)
	if !acquired {
		return nil
	}
	defer runRelease()
	cadenceKey := upstreamBillingProbeLeaderLockKeyAt(lockNow)
	cadenceRelease, acquired := tryAcquireSingletonLeaderLock(ctx, s.lockCache, s.db, cadenceKey, s.instanceID, upstreamBillingProbeLeaderLockTTL)
	if !acquired {
		return nil
	}
	releaseAt := lockNow.Truncate(upstreamBillingProbeCycleInterval).Add(upstreamBillingProbeCycleInterval)
	defer releaseUpstreamBillingProbeLeaderLock(cadenceRelease, releaseAt)

	accounts, err := s.listDueAccounts(ctx, lockNow)
	if err != nil {
	...
	for i := range accounts {
		...
		if snapshot != nil && !snapshot.NextProbeAt.IsZero() && lockNow.Before(snapshot.NextProbeAt) {
			continue
		}
	...
	sort.SliceStable(due, func(i, j int) bool {
		...
		if dueAt := left.NextProbeAt; dueAt.Equal(right.NextProbeAt) {
			return due[i].ID < due[j].ID
		}
...
// ProbeAccount performs one manual probe and ignores the enable switch.
...
func (s *UpstreamBillingProbeService) probeAccountWithMode(ctx context.Context, accountID int64, intervalMinutes int, requireEnabled bool) (*UpstreamBillingProbeSnapshot, error) {
	...
	value, err, _ := s.probeGroup.Do(strconv.FormatInt(accountID, 10), func() (any, error) {
		select {
...
// ProbeAccounts performs a bounded manual batch with the runner concurrency limit.
...
func (s *UpstreamBillingProbeService) SetAccountEnabled(ctx context.Context, accountID int64, enabled bool) error {
	...
	return s.accountRepo.UpdateExtra(ctx, accountID, map[string]any{UpstreamBillingProbeEnabledExtraKey: enabled})
}
