...
//
// fork 里唯一置位 WithOpenAIImagesEndpoint 的地方是
// handleOpenAIImagesErrorResponse（openai_images_responses.go），OAuth 账号的
// /v1/images/* 上游同样是 Codex Responses，也会命中 plan-gated 分支。账号确实
// 不具备生图能力时，冷却是唯一的刹车：调度层靠 model_rate_limits 跳过该账号后
// 快速失败；一旦跳过冷却，每个请求都会完整走一遍号池，对上游形成无上界的 400
// 放大。
...
// 守卫口径必须与冷却键一致：冷却键走 modelRateLimitKeyForUpstreamModelNotFound
// （账号模型映射），账号可以把文本别名映射到 gpt-image-*，只判请求模型会漏掉这种
// 形态，原 bug 原样复现。
