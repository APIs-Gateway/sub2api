...
// fork 回归：原生流式（/v1beta/models/{model}:streamGenerateContent）的每个 chunk
// 都要喂进计数器；累积式流在后续 chunk 重发同一批图，不能重复计费。
func TestHandleNativeStreamingResponse_FeedsImageCounterWithoutDoubleCount(t *testing.T) {
	c := newGeminiImageTestContext(t)
	beginGeminiImageOutputObservation(c)

	twoImages := geminiImageResponse(`{"inlineData":{"mimeType":"image/png","data":"` + geminiTestPNG + `"}},` +
		`{"inlineData":{"mimeType":"image/png","data":"` + geminiTestPNG + `"}}`)
	stream := "data: " + geminiImageResponse(`{"text":"drawing"}`) + "\n\n" +
		"data: " + twoImages + "\n\n" +
		"data: " + twoImages + "\n\n" +
	...
	require.Equal(t, 2, observedGeminiImageOutputs(c))
	require.Equal(t, 2, resolveGeminiImageCount(c, "nana-banana-2", "nana-banana-2"))
}

// fork 回归：Anthropic→Gemini 兼容路径的非流式响应同样按实际回图张数计费（多图记 N 张）。
func TestHandleNonStreamingResponse_FeedsImageCounter(t *testing.T) {
	c := newGeminiImageTestContext(t)
	beginGeminiImageOutputObservation(c)

	body := geminiImageResponse(`{"text":"here"},` +
		`{"inlineData":{"mimeType":"image/png","data":"` + geminiTestPNG + `"}},` +
		`{"inline_data":{"mime_type":"image/jpeg","data":"` + geminiTestPNG + `"}},` +
		`{"inlineData":{"mimeType":"image/webp","data":"` + geminiTestPNG + `"}}`)
	resp := &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}},
	...
	svc := &GeminiMessagesCompatService{}
	usage, err := svc.handleNonStreamingResponse(c, resp, "my-image-alias")
	require.NoError(t, err)
	require.NotNil(t, usage)

	require.Equal(t, 3, resolveGeminiImageCount(c, "my-image-alias", "my-upstream-alias"))
}

// fork 回归：没有任何图片输出的文本请求，即使请求了普通文本模型也保持 0 张，
// 不会因为计数器接线而误计按次费用。
func TestResolveGeminiImageCount_TextStreamStaysZero(t *testing.T) {
	c := newGeminiImageTestContext(t)
	beginGeminiImageOutputObservation(c)

	stream := "data: " + geminiImageResponse(`{"text":"hello"}`) + "\n\n" +
		"data: " + geminiImageResponse(`{"text":"hello world"}`) + "\n\n"
	resp := &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}},
		Body:       io.NopCloser(strings.NewReader(stream)),
