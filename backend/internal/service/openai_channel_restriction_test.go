...
func TestIsUpstreamModelRestrictedByChannel_PassthroughMatchesForwardPath(t *testing.T) {
	...
	// fork 的 ShouldUseResponsesAPI 对"未探测"账号默认走 raw chat completions 兜底
	// （见 upstream_capability.go doc comment），与上游默认假定 unprobed 账号走
	// Responses 不同；显式置 true 才能命中本测试要验证的 passthrough 分支。
	account := &Account{
		Platform: PlatformOpenAI,
