...
type schedulerNilOpenAIAccountRepo struct {
	AccountRepository
}

func (r schedulerNilOpenAIAccountRepo) GetByID(ctx context.Context, id int64) (*Account, error) {
	return nil, nil
}
...
// exits that bypassed the diagnostics already used by the advanced scheduler
// (openai_account_scheduler.go) and the non-batched legacy selector
// (selectBestAccount). Load batching is the default path when it is enabled,
// so quota auto-pause / model mismatches could still surface as an opaque 503
// instead of a diagnosable "filtered: reason=count" summary.
...
func TestOpenAIForcedAccountRoutingContext(t *testing.T) {
	ctx := context.Background()
	var nilCtx context.Context

	require.Zero(t, openAIForcedAccountRoutingID(nilCtx))
	require.True(t, ctx == WithOpenAIForcedAccountRouting(ctx, 0))
	require.True(t, ctx == WithOpenAIForcedAccountRouting(ctx, -1))
	require.Zero(t, openAIForcedAccountRoutingID(context.WithValue(ctx, openAIForcedAccountRoutingContextKey{}, "not-an-id")))
	require.Zero(t, openAIForcedAccountRoutingID(context.WithValue(ctx, openAIForcedAccountRoutingContextKey{}, int64(0))))

	routed := WithOpenAIForcedAccountRouting(ctx, 36013)
	require.False(t, ctx == routed)
	require.Equal(t, int64(36013), openAIForcedAccountRoutingID(routed))
}

func TestOpenAIGatewayService_SelectForcedOpenAIAccountBranches(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()

	ctx := context.Background()
	forcedAccountID := int64(36014)
	...
	t.Run("invalid account id returns plain no available error", func(t *testing.T) {
		svc := &OpenAIGatewayService{cfg: cfg}

		selection, err := svc.selectForcedOpenAIAccount(
			ctx,
			0,
			"",
			nil,
		...
		require.ErrorIs(t, err, ErrNoAvailableAccounts)
		require.Equal(t, ErrNoAvailableAccounts, err)
		require.Nil(t, selection)
	})

	t.Run("missing account does not fall back", func(t *testing.T) {
		svc := &OpenAIGatewayService{
			accountRepo: schedulerNilOpenAIAccountRepo{},
			cfg:         cfg,
		}
	...
	t.Run("capability mismatch rejects forced account", func(t *testing.T) {
	...
	t.Run("busy forced account returns wait plan only for the forced account", func(t *testing.T) {
		svc := &OpenAIGatewayService{
			accountRepo: schedulerTestOpenAIAccountRepo{accounts: []Account{activeAccount}},
			cfg:         cfg,
			concurrencyService: NewConcurrencyService(schedulerTestConcurrencyCache{
				acquireResults: map[int64]bool{forcedAccountID: false},
			}),
		}
		...
		require.Equal(t, forcedAccountID, selection.WaitPlan.AccountID)
		require.Equal(t, activeAccount.Concurrency, selection.WaitPlan.MaxConcurrency)
...
func TestOpenAIGatewayService_SelectAccountWithSchedulerForImages_ServerPolicyRequiresDirectAPIKey(t *testing.T) {
	ctx := context.Background()
	groupID := int64(10107)
	oauth := &Account{ID: 37001, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, GroupIDs: []int64{groupID}}
	apiKey := &Account{ID: 37002, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, GroupIDs: []int64{groupID}, Credentials: map[string]any{"api_key": "sk-test"}}
...
// TestOpenAIGatewayService_SelectAccountForModelWithExclusions_DBRuntimeRecheckRejectsAccountMovedOutOfGroup
// covers a group-isolation race: an account can be present in the scheduler snapshot for the
// requested group at candidate-listing time, then get moved to a different group by an admin
// before the DB recheck runs. Without re-validating group membership against the fresh DB row,
// recheckSelectedOpenAIAccountFromDB would still hand out that account, leaking group-scoped
// traffic to an account that no longer belongs to the group.
func TestOpenAIGatewayService_SelectAccountForModelWithExclusions_DBRuntimeRecheckRejectsAccountMovedOutOfGroup(t *testing.T) {
	ctx := context.Background()
	groupID, otherGroupID := int64(10111), int64(10112)
	stalePrimary := &Account{ID: 34101, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0, GroupIDs: []int64{groupID}}
	staleSecondary := &Account{ID: 34102, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 5, GroupIDs: []int64{groupID}}
	dbPrimary := *stalePrimary
	dbPrimary.GroupIDs = []int64{otherGroupID} // moved out of the requested group after the snapshot was built
	dbSecondary := *staleSecondary
	snapshotCache := &openAISnapshotCacheStub{
		snapshotAccounts: []*Account{stalePrimary, staleSecondary},
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_FailsOpenForSharedQuarantinedProxy(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()
	proxyID := int64(965)
	accounts := []Account{
		{ID: 96501, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, ProxyID: &proxyID},
		{ID: 96502, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 5, ProxyID: &proxyID},
	}
	cfg := &config.Config{}
	cfg.Gateway.Scheduling.LoadBatchEnabled = false
	svc := &OpenAIGatewayService{
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_SkipsQuarantinedSharedProxy(t *testing.T) {
	...
	require.NotNil(t, selection)
	require.NotNil(t, selection.Account)
	require.Equal(t, proxyB, *selection.Account.ProxyID, "legacy scheduling must skip a quarantined proxy when a healthy proxy is available")
}
