...
func TestAPIKeyServiceRejectsOversizedCredentialBeforeRepository(t *testing.T) {
	called := false
	...
	_, err := svc.GetByKey(context.Background(), strings.Repeat("x", MaxAPIKeyCredentialBytes+1))
