...
func (c *openAIProxyStreamCircuit) ensureCapacityLocked(now time.Time) {
	...
	for proxyID, entry := range c.entries {
		if oldestID == 0 || entry.lastTouched.Before(oldest) {
			oldestID = proxyID
