...
//
// fork 独立实现了「省略 stream 字段时默认按流式转发」（避免 CF ~100s 掐断首字节），
// 因此这里必须显式传 stream:false 才能落到本用例要覆盖的非流式分支；
// 省略字段会被 fork 判成 StreamDefaulted=true 直接落到流式分支。
func TestForwardOpenAIImagesAPIKey_NonStreamDetachesUpstreamContext(t *testing.T) {
	body := []byte(`{"model":"gpt-image-2","prompt":"draw a cat","response_format":"b64_json","stream":false}`)
