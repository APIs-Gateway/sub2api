...
type UpdateAPIKeyRequest struct {
	...
	// StablePriorityEnabled per-key 稳定优先开关（nil = 不变）
	StablePriorityEnabled *bool `json:"stable_priority_enabled"`
}
...
func (s *APIKeyService) Update(ctx context.Context, id int64, userID int64, req UpdateAPIKeyRequest) (*APIKey, error) {
	...
	if req.StablePriorityEnabled != nil {
		apiKey.StablePriorityEnabled = *req.StablePriorityEnabled
	}
...
// 返回用户可以选择的标准类型分组：公开的（非专属）或用户被明确允许的。
// burn-down 模型下订阅金额已发放到余额、通过标准渠道消费，订阅型分组不再作为可选渠道返回。
