...
// TrackAPIKeySlot records a best-effort active request slot for an API key.
// It intentionally does not influence user-level concurrency admission.
...
// GetAPIKeyConcurrencyBatch returns best-effort live API key counts. Tracking
// failures must never break a key list or gateway request, so errors become zeroes.
