...
func TestForwardAsChatCompletions_StrictDeveloperRole(t *testing.T) {
	...
	for _, stream := range []bool{false, true} {
		// The fork has no dedicated DeepSeek/Kimi/Zhipu platforms: these
		// providers are OpenAI-compatible API-key accounts, detected by host.
		for _, target := range []struct{ model, base string }{
			{"deepseek-flash", "https://api.deepseek.com"},
			{"k3", "https://api.kimi.com/coding"},
			{"kimi-k3", "https://api.moonshot.ai"},
			{"glm-5.3", "https://open.bigmodel.cn/api/paas/v4"},
		} {
			t.Run(fmt.Sprintf("%s/%s/stream=%t", target.model, target.base, stream), func(t *testing.T) {
...
func TestForwardAsChatCompletions_StrictDeveloperRoleRejectsMalformedMessages(t *testing.T) {
	gin.SetMode(gin.TestMode)
	upstream := &chatRoleContractUpstream{}
	svc := &OpenAIGatewayService{cfg: rawChatCompletionsTestConfig(), httpUpstream: upstream}
	account := rawChatCompletionsTestAccount()
	account.Credentials["base_url"] = "https://api.deepseek.com"
	account.Extra = map[string]any{openai_compat.ExtraKeyResponsesSupported: false}
	body := []byte(`{"model":"deepseek-flash","messages":["secret"],"stream":false}`)
...
func TestNormalizeStrictChatDeveloperRoles_PreservesConversation(t *testing.T) {
	...
	again, err := normalizeStrictChatDeveloperRoles(account, target, got)
	require.NoError(t, err)
...
func TestNormalizeStrictChatDeveloperRoles_OtherTargetsUntouched(t *testing.T) {
	...
	for _, account := range []*Account{nil, {Platform: PlatformOpenAI, Type: AccountTypeOAuth}, {Platform: PlatformGrok, Type: AccountTypeOAuth}} {
