...
func (s *RateLimitService) maybeHandleOpenAITeamLinkedError(ctx context.Context, account *Account, statusCode int, responseBody []byte) {
	...
	// 注：上游此处还会跳过影子账号（ParentAccountID/IsShadow）；fork 的 Account 模型
	// 不存在影子账号概念（见 svc-17 item2 对应说明），因此不需要该过滤条件。
	var targets []*Account
	for i := range accounts {
		acc := &accounts[i]
		if acc.ID == account.ID || strings.TrimSpace(acc.GetChatGPTAccountID()) != teamID {
			continue
		}
