...
type Group struct {
	...
	// 稳定优先下一档兜底目标分组ID（构成多档链），仅 openai 平台生效
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case group.FieldID, group.FieldDefaultValidityDays, group.FieldFallbackGroupID, group.FieldFallbackGroupIDOnInvalidRequest, group.FieldStablePriorityFallbackGroupID, group.FieldSortOrder, group.FieldRpmLimit:
			values[i] = new(sql.NullInt64)
		case group.FieldName, group.FieldDescription, group.FieldStatus, group.FieldDuplicateOperationID, group.FieldPlatform, group.FieldSubscriptionType, group.FieldDefaultMappedModel:
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldStablePriorityFallbackGroupID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field stable_priority_fallback_group_id", values[i])
			} else if value.Valid {
				_m.StablePriorityFallbackGroupID = new(int64)
				*_m.StablePriorityFallbackGroupID = value.Int64
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	if v := _m.StablePriorityFallbackGroupID; v != nil {
		builder.WriteString("stable_priority_fallback_group_id=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
