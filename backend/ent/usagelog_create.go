...
func (_c *UsageLogCreate) defaults() {
	if _, ok := _c.mutation.UpstreamModelMismatch(); !ok {
		v := usagelog.DefaultUpstreamModelMismatch
		_c.mutation.SetUpstreamModelMismatch(v)
	}
...
func (_c *UsageLogCreate) check() error {
	...
	if _, ok := _c.mutation.UpstreamModelMismatch(); !ok {
		return &ValidationError{Name: "upstream_model_mismatch", err: errors.New(`ent: missing required field "UsageLog.upstream_model_mismatch"`)}
	}
