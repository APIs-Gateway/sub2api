...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	subscriptionService := service.NewSubscriptionService(groupRepository, userSubscriptionRepository, userRepository, billingCacheService, apiKeyAuthCacheInvalidator, client, settingService, configConfig)
	...
	stablePriorityStateStore := repository.NewStablePriorityStateStore(redisClient)
	openAIGatewayService := service.NewOpenAIGatewayService(accountRepository, usageLogRepository, usageBillingRepository, userRepository, userSubscriptionRepository, userGroupRateRepository, gatewayCache, configConfig, schedulerSnapshotService, concurrencyService, billingService, rateLimitService, billingCacheService, httpUpstream, deferredService, openAITokenProvider, grokTokenProvider, modelPricingResolver, channelService, balanceNotifyService, settingService, serviceUserPlatformQuotaRepository, stablePriorityStateStore, groupRepository)
	...
	backupHandler := handler.ProvideAdminBackupHandler(backupService, userService, totpService, settingService)
	...
	pointsHandler := admin.NewPointsHandler(pointsService)
	postgreSQLRepository := securityaudit.NewPostgreSQLRepository(db)
	promptEventAdminHandler := securityaudit.NewPromptEventAdminHandler(postgreSQLRepository)
	upstreamBillingProbeService := service.ProvideUpstreamBillingProbeService(accountRepository, accountTestService, settingService, leaderLockCache, db)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, paymentHandler, affiliateHandler, complianceHandler, pointsHandler, promptEventAdminHandler, upstreamBillingProbeService)
	...
	gatewayHandler := handler.ProvideGatewayHandler(gatewayService, geminiMessagesCompatService, antigravityGatewayService, userService, subscriptionService, concurrencyService, billingCacheService, usageService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, userMessageQueueService, configConfig, settingService, openAIGatewayService, coordinator)
	...
	providerPricingHandler := handler.NewProviderPricingHandler(paymentConfigService, pricingService, settingService, groupRepository)
	availableChannelHandler := handler.NewAvailableChannelHandler(channelService, apiKeyService, settingService)
	checkinRepository := repository.NewCheckinRepository(db)
	checkinService := service.NewCheckinService(checkinRepository, settingService, usageLogRepository, billingCacheService, apiKeyAuthCacheInvalidator)
	checkinHandler := handler.NewCheckinHandler(checkinService, turnstileService)
	handlerPointsHandler := handler.NewPointsHandler(pointsService, affiliateService)
	legacyInviteDB, err := repository.InitLegacyInviteDB(configConfig)
	if err != nil {
		return nil, err
	}
	legacyPaidLookup := repository.NewLegacyPaidLookup(legacyInviteDB, configConfig)
	legacyInviteClaimRepository := repository.NewLegacyInviteClaimRepository(db)
	legacyInviteOptions := service.ProvideLegacyInviteOptions(configConfig)
	legacyInviteService := service.NewLegacyInviteService(legacyPaidLookup, legacyInviteClaimRepository, redeemCodeRepository, emailService, settingService, legacyInviteOptions)
	legacyInviteHandler := handler.NewLegacyInviteHandler(legacyInviteService, turnstileService)
	...
	subscriptionExpiryService := service.ProvideSubscriptionExpiryService(userSubscriptionRepository, settingRepository, notificationEmailService, billingCacheService, leaderLockCache, db)
