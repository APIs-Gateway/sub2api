// Command cleanup-ingress-reject-logs removes only historical admission rejects
// that were written to ops_error_logs before ingress rejects gained their bounded,
// sanitized aggregate sink. It intentionally leaves operational and upstream errors.
package main
...
func main() {
	...
	counts, scanned, matched, deleted, err := cleanup(context.Background(), db, before, *batchSize, *execute)
	if err != nil {
