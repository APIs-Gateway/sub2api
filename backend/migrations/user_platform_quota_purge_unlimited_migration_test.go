...
// TestUserPlatformQuotasPurgeUnlimitedMigration 校验 192 号迁移（上游编号 238，按 fork 下一个
// 空闲编号改号）只删除三档限额全为 NULL 的行：
// 这类行等价于"不存在"，任何一档非 NULL 的记录（含软删历史）都必须保留。
func TestUserPlatformQuotasPurgeUnlimitedMigration(t *testing.T) {
	content, err := FS.ReadFile("192_purge_unlimited_user_platform_quotas.sql")
